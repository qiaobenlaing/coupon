...
class BmStaffController extends AdminBaseController {
    ...
    public function homepage() {
        ...
        $countShop = $shopMdl->countShop();
        // 获得开通支付商户个数
        $onlinePayShopCount = $shopMdl->countShop(array('isAcceptBanKCard' => C('YES')));
    ...
    public function login() {
        ...
        if (IS_GET) {
            $bankList =  D("bank")->where(array("status"=>1,"parent"=>array("not IN","-1")))->field("name,id")->select();
            $assign = array(
                "bankList"=> $bankList
            );
            $this->assign($assign);
            ...
            $ret = $baseMdl->login($mobileNbr, $password, $bank_id,$loginType);
            if ($ret === true) {
    ...
    public function listBmStaff() {
        ...
        //获取管理员所属平台
        foreach($bmStaffList as $k => &$v){
            if($v['bank_id'] == -1){
                $v['pingtaiName'] = '总平台';
            }else{
                $pingtaiName = M('bank')->where(array('id' => $v['bank_id']))->getField('name');
                $v['pingtaiName'] = $pingtaiName;
            }
        }
