...
class ShopController extends AdminBaseController {
    ...
    public function listShop() {
        ...
        foreach($shopList as $k => $shop) {
            ...
            // 获得商户的所属类型
            $zoneId['shopCode'] = $shop['shopCode'];
            $shopList[$k]['shopType'] = $shopTypeRelMdl->getShopType2($zoneId);
            $shopList[$k]['bank_name'] = getName("Bank","name","id='".$shopList[$k]['bank_id']."'");
        }

        $shopCount = $shopMdl->countShop($condition);

        $shopTypeMdl = new ShopTypeModel();
        $where_bank_id['bank_id'] = $zoneId['bank_id'];
        $shopTypeList = $shopTypeMdl->getAllShopTypeList2($where_bank_id);
    ...
    public function editShop() {
        ...
        if(IS_GET) {
            ...
            if(!empty($shopInfo['logoUrl'])){
                $shopInfo['logoUrl'] = C("urlOSS").$shopInfo['logoUrl'];
            }

            // 获得商户所属类型
            $where1 = array("shopCode"=>$shopInfo['shopCode']);
            if($_SESSION['USER']['bank_id']!=-1){
                $where1['bank_id'] = $_SESSION['USER']['bank_id'];
            }

            $shopType = $shopTypeRelMdl->getShopType2($where1);
            ...
            $search = array();
            if($_SESSION['USER']['bank_id']!=-1){
                $search['bank_id'] = $_SESSION['USER']['bank_id'];
            }

            // 获得所有商户类型
            $shopTypeMdl = new ShopTypeModel();
            $shopTypeList = $shopTypeMdl->field(array('shopTypeId', 'typeValue', 'typeZh'))->where($search)->select();

            // 获得所有品牌
            $brandMdl = new BrandModel();
            $brandList = $brandMdl->getBrandList($search, array('*'));
            ...
            if($_SESSION['USER']['bank_id']!=-1){
                $search['bank_id'] = $_SESSION['USER']['bank_id'];
                $parentList = $shopMdl->getShopList(array('shopCode' => array('NOTIN', $shopCodeArr), 'parentCode' => '',"bank_id"=>$search['bank_id']), array('shopCode', 'shopName'));
            }else{
                $parentList = $shopMdl->getShopList(array('shopCode' => array('NOTIN', $shopCodeArr), 'parentCode' => ''), array('shopCode', 'shopName'));
            }
            ...
            $districtMdl = new DistrictModel();
            // 获得浙江省的所有市级地及其地区号
            $areaNbrList = $districtMdl->getCityList('');
...
//            }
            ...
            //判断是否为惠圈管理人员
                if($_SESSION['USER']['bank_id']==-1){
                    $bankList = D("bank")->where("status=1 and parent!=-1")->select();
                }else{
                    $bankList = D("bank")->where("status=1 and id = '".$_SESSION['USER']['bank_id']."' and parent!=-1")->select();
                }


            //商家装饰
            if(!empty($shopInfo['decoration']) && is_array($shopInfo['decoration'])){
                foreach ($shopInfo['decoration'] as  $item  =>&$value){
                    $value['imgUrl'] = C("urlOSS").$value['imgUrl'];
                }
            }
            ...
            $data = I('post.');
            // 判断是否是OSS显示的图片（保存）
            if(strpos($data['logoUrl'],C("urlOSS"))!==false){
                //截取掉前面的https://oss.cloud.hfq.huift.com.cn/
                $start = stripos($data['logoUrl'],C("urlOSS"));
                ...
                $data['logoUrl'] = substr($data['logoUrl'],$length-1,mb_strlen($data['logoUrl'])-$length+1);
            }
...
//            $this->ajaxError('', $data);
            ...
            //判断是否为数组
            if(!empty($imgUrlArr) && is_array($imgUrlArr)){
                $imgArr = array();
                foreach ($imgUrlArr as $value){
                    $start = stripos($value,C("urlOSS"));
                    $length = mb_strlen(C("urlOSS"));
                    $value = substr($value,$length-1,mb_strlen($value)-$length+1);
                    $imgArr[] = $value;
                }
                $imgUrlArr = $imgArr;
            }
...
//        $info = $upload->upload(); // 上传图片
        ...
        //判断文件是否上传有误
        if($upFile['error']>0){
            $errinfo = array("info"=>$_FILES["file"]["error"]);
        }

        //1.判断文件上传类型
        $allowType = array("image/jpeg","image/png","image/gif","image/bmp","image/swf");
        $type = $upFile['type'];
        if(!in_array($type, $allowType)){
            $errinfo = array("info"=>"文件类型不支持");
        }

        //2.判断图片大小
        $fileSize = $upFile['size'];
        if($fileSize>45240000){
            $errinfo = array("info"=>"图片太大");
        }
        ...
        //防止文件重名
        $filename =rand(1000,9999).time().$upFile['name'];
        $path = $dir.$filename;

        //判断文件是否存在
        if(file_exists($path)){
            $errinfo = array("info"=>"文件已存在");
        }

        if(!empty($errinfo)) {
            // 上传错误提示错误信息
            echo json_encode(array('code' => $errinfo['info']));
            exit;
        } else {
            ...
            try {
                ...
                $ossClient->uploadFile($bucket,$path,$upFile["tmp_name"]);


            } catch (OssException $e) {
                $mess = $e->getMessage();
                echo json_encode(array("code"=>$mess));
                exit;
            }
...
//                $res = $shopDecorationMdl->addShopDecoration($shopCode, '', '', '', $url, '');
                ...
                if($upFile['type'] == 'image/jpeg') {
                    $data['id'] = substr($filename, 0, -5);
                } else {
                    $data['id'] = substr($filename, 0, -4);
                }
...
//                }
    ...
    public function listAlbumPhoto(){
        if(IS_GET) {
            ...
            // 判断是否是OSS显示的图片（显示）
            foreach ($subAlbum['photo'] as $item=>&$value){
                if(!empty($value['url'])){
                    $value['url'] = C("urlOSS").$value['url'];//添加域名前缀
                }
            }
...
//            $objActSheet->getStyle("$letter[$i]$head_row")->getFont()->setSize(16);
    ...
    public function barCodeList(){
        ...
        if(!empty($get['shopCode'])){
            $where['shopCode'] = $get['shopCode'];
        }
        if(!empty($get['shopName'])){
            $where['shopName'] =array("LIKE", '%'.$get['shopName'].'%');
        }
        $where = array();
        if($_SESSION['USER']['bank_id']!=-1){
        ...
        $shopList = M('shop')->field('shopCode,shopName,status,shopStatus,barCodeUrl')->where($where)->select();
        $shopCount = count($shopList);
        foreach ($shopList as $item => $value){
            if(!empty($value['barCodeUrl']))
                $shopList[$item]['barCodeUrl'] = C("urlOSS").$value['barCodeUrl'];
            $shopList[$item]['explain'] = C('BARCODE.EXPLAIN');
        }
        $this->pager->setItemCount($shopCount);
        $assign = array(
            'title' => '商户付款码列表',
    ...
    public function addShopBarCode(){
        if(IS_AJAX){
            $data = I('post.');
            $png = $this->createCode($data['shopCode'],$data['logoUrl']);
            if($png['status'] = 'success')
                echo json_encode(array('status' => '1','msg' => '二维码生成成功','html' => $png['url']));
            else
                echo json_encode(array('status' => '2','msg' => '二维码生成失败',''));
            ...
            if($_SESSION['USER']['bank_id']!=-1){
                $zoneId['bank_id'] =  $_SESSION['USER']['bank_id'];
                $where['bank_id'] = array('eq',$zoneId);
            }
            $where['status'] = array('eq',1);
            $where['shopStatus'] = array('eq',2);
            $where['shopCode'] = $shopCode;
            $shopModel = new ShopModel();
            $shopInfo = $shopModel->where($where)->field('shopCode,shopName,logoUrl,barCodeUrl')->find();
            $shopInfo['barCodeUrl'] = C("urlOSS").$shopInfo['barCodeUrl'];
            $assign = array(
                'title' => '商户生成付款码信息',
                'shopInfo' => $shopInfo,
            );
    ...
    protected function createCode($data,$logo){
        Vendor('QRcode.phpqrcode');
        $errorCorrectionLevel = 'L';//容错级别
        $matrixPointSize = 6;//生成图片大小
        //生成二维码图片
        \QRcode::png($data, '/home/wwwroot/gzyh.hfq.huift.com.cn/Public/img/barCodePng/qrcode.png', $errorCorrectionLevel, $matrixPointSize, 2);
        $logo = C("urlOSS").$logo;
        $QR = '/home/wwwroot/gzyh.hfq.huift.com.cn/Public/img/barCodePng/qrcode.png';//已经生成的原始二维码图
        $codePath = $QR;
        if ($logo !== FALSE) {
            ...
            //输出图片
        imagepng($QR, '/home/wwwroot/gzyh.hfq.huift.com.cn/Public/img/barCodePng/helloweba.png');
        unlink($codePath);
        $path = '/home/wwwroot/gzyh.hfq.huift.com.cn/Public/img/barCodePng/helloweba.png';
        $upLoadFileToOssObj = new UtilController();
        $res = $upLoadFileToOssObj->barCodeImageUp($path);
        $saveRes = $this->saveBarCodePngUrl($data,$res);
        if($saveRes['status'] == 'error')
            return $saveRes;
        else{
            $url = C("urlOSS").$res;
            return array('status' => 'success','msg' => '二维码生成成功','url' => "<img src='$url'>");
        }
    }
    protected function saveBarCodePngUrl($shopCode,$pngUrl){
        if(!$shopCode || !$pngUrl)
            return array('status'=>'error','msg'=>'参数错误');
        //删除oss上面旧的二维码图片文件
        $barCodeUrl = M('shop')->where(array('shopCode' => $shopCode))->getField('barCodeUrl');
        if($barCodeUrl != ''){
            $upLoadFileToOssObj = new UtilController();
            $res = $upLoadFileToOssObj->delCodeImage($barCodeUrl);
            if($res != 'success')
                return array('status' => 'error','msg' => '数据添加失败');
        }
        $res = M('shop')->where(array('shopCode'  => $shopCode))->setField('barCodeUrl', $pngUrl);
        if($res)
            return array('status' => 'success','msg' => '数据添加成功');
        else
            return array('status' => 'error','msg' => '数据添加失败');
    }
    public function delCodeImage(){
        if(!IS_AJAX)
            return false;
        $path = I('get.url','');
        if(!$path)
            return false;
        $object = strchr($path,'Public');
        $upLoadFileToOssObj = new UtilController();
        $res = $upLoadFileToOssObj->delCodeImage($object);
        if($res['status'] == 'success'){
            //清空shop表里面的barCodeUrl
            M('shop')->where(array('barCodeUrl'=> $object))->setField('barCodeUrl','');
            echo json_encode($res);
        }
        else
            echo json_encode(array('status' => 'error','msg' => '删除出错'));
    }
}
