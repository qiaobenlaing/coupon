...
class ActivityController extends AdminBaseController {
    ...
    public function listSpActivity() {
        ...
        $filterData['Activity.isShow'] = array('eq',1);
...
//        unset($filterData['status']);
    ...
    public function editActivity() {
        ...
        if (IS_GET) {
            ...
            //获取活动对应优惠券信息
            $couponInfo = M('couponactivity')
                ->alias('a')
                ->field('a.function,b.couponName,b.batchCouponCode,b.couponType')
                ->join('LEFT JOIN batchcoupon b ON b.batchCouponCode=a.batchCouponCode')
                ->where(array('a.activityCode' => $activityCode))
                ->find();
            //获取管理端商家信息
            $getShopName = $shopMdl->getManagerShopName();
            //获取商店优惠券
            $coupons = M('batchcoupon')
                ->field('couponName,batchCouponCode,couponType,function')
                ->where(array('shopCode' => array('eq',$actInfo['shopCode']),'couponType' => array('in','7,8'),'expireTime' => array('gt',date('Y-m-d H:i:s',time()))))
                ->select();
            $assign = array(
...
//            $this->ajaxError('', $data);
            ...
            //判断活动类型，如果活动类型不是其他活动，优惠券设置为空；
            if($data['actType'] != 7)
                $data['batchCouponCode'] == '';
    ...
    /*c
     * ajax请求获取某商店的代金券和兑换券数据（其它券价格为0，没有必要参加活动）
     * */
    public function ajaxGetCoupon(){
        $shopCode = I('get.shopCode');
        if($shopCode){
            $couponData = M('batchcoupon')
                ->field('couponName,batchCouponCode,couponType,function')
                ->where(array('shopCode' => array('eq',$shopCode),'couponType' => array('in','7,8'),'expireTime' => array('gt',date('Y-m-d H:i:s',time()))))
                ->select();
            $this->ajaxReturn($couponData);
        }else{
            echo '参数错误';
        }
    }
    /*
 * ajax处理某条活动记录，让其不在后台显示(假删除）
 * */
    public function  setHidden(){
        $activityCode = I('get.activityCode');
        if($activityCode){
           $res = M('activity')->where(array('activityCode' => $activityCode))->setField('isShow',0);
            if($res)
