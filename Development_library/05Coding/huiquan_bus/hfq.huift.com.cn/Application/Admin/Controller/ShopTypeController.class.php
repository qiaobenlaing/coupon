...
class ShopTypeController extends AdminBaseController {
    ...
    public function editShopType() {
        ...
        if(IS_GET){
            ...
            // 判断是否是OSS显示的图片（保存）
            if(strpos($data['shopTypeImg'],C("urlOSS"))!==false){
                ...
                $data['shopTypeImg'] = substr($data['shopTypeImg'],$length-1,mb_strlen($data['shopTypeImg'])-$length+1);
            }

            $ret = $shopTypeMdl->editShopType($data);
            if($ret['status'] == 'error'){
                $this->ajaxError($ret['res']);
                exit;
            }

            $cityShopTypeMdl = new CityShopTypeModel();
            if(empty($data['shopTypeId']) && $ret['status'] == 'success'){
                $data['shopTypeId'] = $ret['res'];
            }
            $res =   $cityShopTypeMdl->addLinkedCity($data['shopTypeId'], $linkedCity);
            if($res){
                $this->ajaxSucc("添加成功");
            }else{
                $this->ajaxError('添加城市出错');
            }
        }
    }
}
