            ...
            if($where['bank_id']!=""){
                ...
                //没查商圈查了地区
                    $list = $this->table('District as d1')
                        ...
                        ->order('d1.parentId asc')
                        ->where("d2.name='".$where['d2.name']."' and d2.isOpen='1'")
                        ->select();
...
//            判断是否选择了所属商圈
    ...
    public function getCityList($where,$pager){
        ...
        if(isset($where['name'])){
            $parentId = $this->where(array('name' => $where['name']))->getField('id');
            $childrenCity = array_column($this->field('id')->where(array('parentId' => $parentId))->select(),'id');
            $where['id'] = array('in',$childrenCity);
            unset($where['name']);
        }
        if($where['isOpen'] == 1){
            foreach ($where as $k => $v){
                if($k == 'isOpen' || $k == 'bank_id')
                    $map['b.'.$k] = $v;
                else
                    $map['a.'.$k] = $v;
            }
            ...
            $cityInfo = $this
                ->alias('a')
                ->field('a.name,a.id,b.isOpen,a.parentId,b.bank_id')
                ->join('district_bank_id b on a.id = b.dist_id')
                ->where($map)
                ->page($pager)
                ->select();
            $cityCount = $this
                ...
                ->where($map)
                ->count();
            return array('cityList' => $cityInfo,'cityCount' => $cityCount);
        }else{
            //获取已开通城市id
            $where['isOpen'] = 1;
            if(isset($where['id'])){
                $where['dist_id'] = $where['id'];
                $cityIdList = $where['id'];
                unset($where['id']);
            }
            $idList = M('district_bank_id')
                ->field('dist_id')
                ->where($where)
                ->select();
            if(!empty($idList)){
                if(isset($where['dist_id'])){//去除已开通城市id
                    $cityIdList = array('in',array_diff($where['dist_id'][1],array_column($idList,'dist_id')));
                    unset($where['dist_id']);
                }
            }
//            $map['isOpen'] = 0;
            ...
            foreach($cityInfo as $k => $v){
                $cityInfo[$k]['isOpen'] = 0;
                $cityInfo[$k]['bank_id'] = isset($where['bank_id']) ? $where['bank_id'] : '';
            }
            $cityCount = $this
                ->field('name,id,parentId')
                ->where($map)
