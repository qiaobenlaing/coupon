...
//        //判断6月29至9月30其中某一天是否是星期六
//        if(($usertime> strtotime('2019-06-29') && $usertime <strtotime('2019-09-30'))){
//            if(date('w')!='6'){
//                return array('code'=>9377);//未到优惠券领取时间
//            }
//        }

        //判断用户是否绑定手机号
        $userModel = new UserModel();
        $mobile =   $userModel->getUserInfo(array('userCode'=>$userCode),array('mobileNbr'));
        if($mobile['mobileNbr']==-1){
            return array('code'=>9527);//没绑手机号
        }

        $batchCouponInfo = $this->getCouponBasicInfo(array('batchCouponCode' => $batchCouponCode), array('endTakingTime', 'dayEndUsingTime', 'nbrPerPerson', 'remaining', 'totalVolume','exRuleList'));

        //判断优惠券是否是持卡优惠券
//        if(!empty($batchCouponInfo['exRuleList'])){
//            $userModel = new UserModel();
//            $info = $userModel->getUserInfo(array('userCode'=>$userCode));
//            //判断用户手机号是否存在
//            if($info && $info['mobileNbr']!='-1'){
//                //判断用户是否是持卡用户
//                $payload = array();
//                $jwt = new Jwt();
...
//
//                $data = array(
//                    'mobile'=> $info['mobileNbr'],
//                    'token'=>$token
//                );
//                //请求第三方的判断接口
//                $info = $this-> post_json_data("http://spark.weidingzhi.net:9076/spark/bank/isPossessCardUser",$data);
//                $result2 = $info['result'];
//                $api_info = json_decode( $result2,1);
//                if($api_info['result_code']!='000'){
//                    return array('code'=>9422);//非持卡人无法领取持卡优惠券
//                }
//            }
//        }
...
//        }
    ...
    public function editBatchCoupon($couponInfo) {
        ...
        if($this->validate($rules)->create($couponInfo) != false) {
            ...
            // 各种价格单位：元化单位：分
            $couponInfo = $this->byHundred($couponInfo, array('insteadPrice', 'availablePrice', 'canTakePrice', 'sendRequired', 'payPrice','topAmount'));
...
//            return $this->getBusinessCode($code);
    ...
    public function listAvailabelCoupon($shopCode, $field, $userCode = '',$longitude,$latitude) {
        ...
        //gzbank需求，判断用户是否为持卡人，不是，则显示一般优惠券，是则连同特殊优惠券一起显示
        //判断用户是否是持卡用户
//        $expectArr = array();
//        $userModel = new UserModel();
//        $info = $userModel->getUserInfo(array('userCode'=>$userCode));
//        //判断用户手机号是否存在
//        $isHaveCard['isHaveCard'] = 2;
//        if($info && $info['mobileNbr']!='-1'){
//            //判断用户是否是持卡用户
//            $payload = array();
//            $jwt = new Jwt();
...
//            $api_info = json_decode( $result2,1);
//            if($api_info['result_code']=='000'){
//                $isHaveCard['isHaveCard'] = 1;//非持卡人无法领取持卡优惠券
//            }
//        }

//        if($isHaveCard['isHaveCard']!=1){
//            $expectCouponList  =   $this->field('batchCouponCode,exRuleList')->select();
//            foreach ($expectCouponList as $item => $value){
//                if(!empty($value['exRuleList']) && $value['exRuleList'] != ''){
//                    $expectArr[] = $value['batchCouponCode'];
//                }
//            }
//        }
//        $where['batchCouponCode'] = array('not in',$expectArr);
//        //如果是持卡人允许领取所有优惠券
//        if(empty($expectArr)){
//            unset($where['batchCouponCode']);
//        }
...
//                    }
     ...
     * @return array $condition
     */
    public function getCondition($city, $searchWord, $userCode,$couponType) {
...
//	'CollectInfo'=>$collectCoupon,
    ...
    public function listCoupon2($couponType, $searchWord, $longitude, $latitude, $page, $city, $userCode,$zoneId) {
        $condition = $this->getCondition($city, $searchWord, $userCode,$couponType);
        $couponList= $this->field(array(
            'batchCouponCode',
            'insteadPrice',
            'discountPercent',
        ...
        if($couponList) {
            ...
            foreach($couponList as $k => &$v) {
                ...
                $couponList[$k]['availablePrice'] = $v['availablePrice'] / C('RATIO');
                $couponList[$k]['payPrice'] = $v['payPrice'] / C('RATIO');
                //c判断该优惠券是否参加活动，且活动有效并正在进行，参加活动价格进行更改，但是这样改的话只能是单个优惠券进行的活动，不能做捆绑的活动；
                $couponActivityData = M('couponactivity')
                    ->field('a.function,a.activityCode')
                    ->alias('a')
                    ->join('LEFT JOIN __ACTIVITY__ b ON a.activityCode = b.activityCode')
                    ->where(array('a.batchCouponCode' => $v['batchCouponCode']))
                    ->where(array('b.status' =>array('eq',1),'b.isShow' => array('eq',1),'b.startTime' => array('elt',date('Y-m-d H:i:s',time())),'b.endTime' => array('egt',date('Y-m-d H:i:s',time()))))
                    ->find();
                if($couponActivityData && !empty($couponActivityData['function'])){
                    $activityMdl = new ActivityModel();
                    $payPrice = $activityMdl->$couponActivityData['function']($v['payPrice']*100,$couponNum=1,$type=true);
                    $couponList[$k]['activityCode'] = $couponActivityData['activityCode'];//活动码
                    $couponList[$k]['oldPrice'] = $v['payPrice'];//活动前价格
                    $couponList[$k]['payPrice'] = $payPrice['newPrice'] ;
                }
              if($v['exRuleList']) {
                    $v['exRuleList'] = json_decode($v['exRuleList']);
                    $exRuleDesList = $couponRuleMdl->listCouponRule(array('ruleCode' => array('IN', $v['exRuleList'])), array('ruleDes'));
     ...
     * @return int
     */
    public function countCoupon2($couponType, $searchWord, $longitude, $latitude, $city,$userCode,$zoneId) {
        $condition = $this->getCondition($city, $searchWord,$userCode,$couponType);
        $couponCount = $this
            ->join('Shop on Shop.shopCode = BatchCoupon.shopCode and Shop.bank_id='.$zoneId)
            ->join('BackgroundTemplate on BackgroundTemplate.bgCode = BatchCoupon.bgImgCode')
