...
class ShopTypeModel extends BaseModel {
    ...
    public function editShopType($data) {
        ...
        $rules = array(
            ...
            array('typeValue', '' , "类型值已经存在,请重新填写", 1, 'unique'),
            array('typeValue', 0 , "类型值不能为0", 1, 'notequal'),
            array('typeZh', 'require', C('SHOP_TYPE.TYPEZH_EMPTY')),
            array('typeZh', '', '类型描述已经存在',1,'unique'),
        );
        if($this->validate($rules)->create($data) != false) {
            ...
            return array('status' => 'success','res' => $ret);
        } else {
            return array('status' => 'error','res' => $this->getError());
        }
    }
