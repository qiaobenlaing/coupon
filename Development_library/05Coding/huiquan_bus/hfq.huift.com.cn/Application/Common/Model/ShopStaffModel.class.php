...
class ShopStaffModel extends BaseModel {
    ...
    public function getShopStaffList($condition, $field) {
        ...
        if($_SESSION['USER']['bank_id']!=-1){
            $condition['bank_id'] = $_SESSION['USER']['bank_id'];
        }
    ...
    public function listShopStaff($filterData, $page) {
        ...
        //判断是否为惠圈管理人员
        if($_SESSION['USER']['bank_id']!=-1) {
            $where['ShopStaff.bank_id'] = $_SESSION['USER']['bank_id'];
        }

        return $this
                ->field(array('DISTINCT(ShopStaff.staffCode)', 'ShopStaff.mobileNbr' => 'mobileNbr', 'realName', 'wechatId', 'userLvl', 'ShopStaff.status' => 'status', 'avatarUrl', 'ShopStaff.brandId','ShopStaff.bank_id', 'ShopStaff.parentCode', 'isSendPayedMsg', 'ShopStaff.access'))
    ...
    public function addStaff($staffInfo) {
        ...
        if($this->validate($rules)->create($staffInfo) != false) {
            ...
            //判断是否为惠圈管理人员
            if($_SESSION['USER']['bank_id']!=-1) {
                $staffInfo['bank_id'] = $_SESSION['USER']['bank_id'];
            }
            ...
            $shopStaffInfo = array(
                'staffCode' => $staffCode,
                'bank_id' => isset($staffInfo['bank_id']) ? $staffInfo['bank_id'] :'',
    ...
    public function editShopStaff($data) {
        ...
        //判断是否为惠圈管理人员
        $where_mobile_Nbr= array("staffCode"=>$data['staffCode']);
        if($_SESSION['USER']['bank_id']!=-1) {
            $data['bank_id'] = $_SESSION['USER']['bank_id'];
            $where_mobile_Nbr['bank_id'] = $data['bank_id'];
        }

        if($this->validate($rules)->create($data)) {
            unset($data['cfPassword']);
            $staffInfo = $this->field('staffCode')->where($where_mobile_Nbr)->find();
            if($data['staffCode'] && $staffInfo) {
                $shopStaffInfo  = $this->getShopStaffInfo(array('mobileNbr' => $data['mobileNbr'],"bank_id"=>$data['bank_id']));
