...
class JpushTestController extends Controller {
    /**
     * 模拟post进行url请求
     * @param string $url
     * @param string $param
    ...
    public function jpush($data)
    {
        ...
        //(必填)待发送的应用程序(appKey)
        $appkeys = 'f25791f60bbb4c5f3676b3fe';

        //(必填)API MasterSecret
        $masterSecret = '85d76db160a78a40a6bbf7e6';
        ...
        $content['n_title']      = '云瞭望'; //（可选）通知标题。不填则默认使用该应用的名称。只有 Android支持这个参考。
        $content['n_content']    = $data['push_title']; //（必填）通知内容。
