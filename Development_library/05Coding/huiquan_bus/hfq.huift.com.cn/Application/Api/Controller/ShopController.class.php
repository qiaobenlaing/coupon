...
class ShopController extends ApiBaseController {
	...
	//显示商家收藏列表
    public function collectShop($userCode,$longitude,$latitude,$zoneId){

		$CollectShopModel=D('CollectShop');
	   $shop=D('Shop');
        //查询优惠券编码和客户是否收藏
        $colle=$CollectShopModel->field(array('isCollect'))
            ->join("User on User.userCode=CollectShop.userCode")
            ->where("User.userCode='$userCode' and User.bank_id=$zoneId")
            ->select();

			 $isHave=$CollectShopModel->join("User on User.userCode=CollectShop.userCode")->where("isCollect=1 AND CollectShop.userCode='$userCode' and User.bank_id=$zoneId")->count();
         ...
         if($value['isCollect']==1){
			...
			'CollectShop.isCollect',
			 ...
			 ->join('CollectShop on CollectShop.shopCode=Shop.shopCode','Left')
                 ->join("User on User.userCode=CollectShop.userCode")
			->where("User.userCode='$userCode' And isCollect=1 and User.bank_id=$zoneId")
			->select();
     ...
     * 显示商家是否收藏,客户点击收藏商家按钮
     */
    public function showCollect($shopCode,$userCode,$zoneId){
		 $arr=array(
            'userCode'=>$userCode,
            'shopCode'=>$shopCode,
		...
		$shopCount=  $CollectShopModel->join("User on User.userCode=CollectShop.userCode")->where("CollectShop.userCode='$userCode' AND shopCode='$shopCode' and User.bank_id=$zoneId")->count();
		if($shopCount<1){
        ...
        $isCollect=$CollectShopModel->field("isCollect")
            ->join("User on User.userCode=CollectShop.userCode")
            ->where("CollectShop.userCode='$userCode' AND shopCode='$shopCode' and User.bank_id=$zoneId")
            ->select();
        ...
        foreach ($isCollect as $value){//1收藏  0没收藏
            if($value['isCollect']==1){
					...
					$CollectShopModel->where("CollectShop.userCode='$userCode' AND shopCode='$shopCode'")->save();
					$isCollect=0;
            }else{
     ...
     * 显示客户是否收藏，客户点击收藏优惠券
     */
	public function collectCoupon($batchCouponCode,$userCode,$zoneId){
		...
		$couponCount=$CollectCouponModel->join("user on user.userCode=collectcoupon.userCode")->where("collectCouponCode='$batchCouponCode' AND user.userCode='$userCode' and user.bank_id=$zoneId")->count();
		...
		//查询isCollect是否为1收藏
		$isCollect=$CollectCouponModel->field("collectcoupon.isCollect")
                                    ->join("user on user.userCode=collectcoupon.userCode")
									->where("collectCouponCode='$batchCouponCode' AND user.userCode='$userCode' and user.bank_id=$zoneId")
									->select();
     ...
     * @return array
     */
	public function showCoupon($userCode,$zoneId){
		 $CollectCouponModel=D('CollectCoupon');
		 $batchCoupon=D('BatchCoupon');
        //查询优惠券编码和客户是否收藏
        $colle=$CollectCouponModel->field(array('isCollect','collectCouponCode'))
            ->join("User on User.userCode=CollectCoupon.userCode")
            ->where("CollectCoupon.userCode='$userCode' and User.bank_id=$zoneId")
            ->select();

			 $isHave=$CollectCouponModel->join("User on User.userCode=CollectCoupon.userCode")->where("isCollect=1 AND CollectCoupon.userCode='$userCode' and User.bank_id=$zoneId")->count();
			if($isHave<1){
				return "null";
			}else{
        foreach ($colle as $value){//1收藏  2没收藏
		...
		->where("collectCouponCode='$collectCouponCode' And CollectCoupon.userCode='$userCode'")
        ->select();
		//分转换成元
	...
	public function getUserCouponNbr($userCode,$batchCouponCode){
        ...
        if(!empty($userCode)&&!empty($batchCouponCode)){
            $userCouponNbr=$UserCouponModel->field(array('a.userCouponNbr','a.status','a.userCouponCode',"a.inWeixinCard","b.costMoney"))
                ->alias('a')
                ->join('LEFT JOIN __ORDERCOUPON__ b ON a.userCouponNbr = b.couponCode')
                ->where(array('a.batchCouponCode' => $batchCouponCode,'a.userCode' => $userCode))
                ->select();
            foreach($userCouponNbr as &$v){
                //分化元
                $v['costMoney'] = $v['costMoney']/100;
            }
        }
     ...
     *    查询优惠券是否收藏，领取
     */
	public function isCollectGet($userCode,$zoneId){
		$CollectCouponModel=D('CollectCoupon');
		...
		'isGetCount'
            ...
            ->join("User on User.userCode=CollectCoupon.userCode")
		->where(array("User.userCode"=>$userCode,"User.bank_id"=>$zoneId))
		->select();
     ...
     * @return array
     */
	public function isBindMobile($userCode){
	    $userModel = new UserModel();
	    $userInfo = $userModel->getUserInfo(array('userCode'=>$userCode),array('mobileNbr'));
	    if($userInfo['mobileNbr']!='-1' && $userInfo){
            return array('code'=>50000,'mobileNbr'=>$userInfo['mobileNbr']);
        }else{
	 ...
	 */
	 public function  getBachCouponInfo($batchCouponCode,$userCode){
		 ...
		 //获取优惠券付款时的价格
//         $couponData = M('usercoupon')
//             ->alias('a')
//             ->field('a.userCouponNbr,b.costMoney')
//             ->join('LEFT JOIN __ORDERCOUPON__ b ON a.userCouponNbr = b.couponCode')
//             ->where(array('a.batchCouponCode' => $batchCouponCode,'a.userCode' => $userCode))
//             ->select();
         //分化为元
...
//         return $couponData;
	...
	function http_curl($url,$type='get',$res='json',$arr=''){
		...
		//获取用户基本信息
	function  getUserInfo($openId,$access_token,$zoneId){
		//根据openId和bank_d查询有没有客户信息
        $where = array("openId"=>$openId,"bank_id"=>$zoneId);

		$userModel=D('User');
		$user=new UserModel();
		$OpenIdCount=$userModel->where($where)->find();
		...
		if(!empty($OpenIdCount)){
			...
			if(!isset($access_token)){
                ...
                //查询是否有openId
                $User=D('User');
                $OpenIdCount=$User->where($where)->count();
                $user=new UserModel();
                $userCode=$user->create_uuid();
                if($OpenIdCount<1){
                     $postArr2=array(
	...
	//优惠券轮播图
	function couponSwiper($city,$zoneId){
        // 获取用户所在商圈
        $bank_id = $zoneId;
		$swiperModel=D('Swiper');
		$arrCouponImgInfo=$swiperModel->field(array('url','title','activityWeb'))->where("city='$city' and bank_id='$bank_id'")->select();
	...
	//c
	//获得会员卡列表
    function showCardList($userCode,$zoneId) {
         //获取商圈内的店铺
        $shopCode = M('shop')->where(array('bank_id'=>$zoneId,'status'=>1))->getField('shopCode',true);
        if ($shopCode){
            $cardCodeList = M('Usercard')->field('cardCode')->where(array('userCode' => $userCode,'status'=>1))->where(array('shopCode'=>array('in',$shopCode)))->select();
            if($cardCodeList){
//             $cardCode = array_column($cardCodeList,'cardCode');
//             $UserCard = M('Card')->field('*')->where(array('cardCode' => array('in',$cardCode)))->select();
                foreach($cardCodeList as $v){
                    $UserCard[] = M('Card')->field('a.*,b.shopName')->alias('a')->join('__SHOP__ b on a.shopCode=b.shopCode')->where(array('a.cardCode' => $v['cardCode']))->find();
                }
                return $UserCard;
            }else{
                return '暂无可用会员卡';
    ...
    function getAllPoint($userCode,$zoneId){
        $allPoint = M('pointearninglog')->field('SUM(point) as allPoint')->where(array('userCode'=>$userCode,'zone'=>$zoneId))->select();
        if($allPoint){
            return $allPoint;
        }else{
            return '暂时还没有可用圈值';
    ...
    function getActivities($shopCode){
        $shopActive = M('activity')->where(array('shipCode' => $shopCode,'status'=>1))->select();
        if ($shopActive){
            return $shopActive;
        }else{
            return '暂无活动';
    ...
    //获取某商圈内的商家活动,$getField:需要的表字段
    protected  function activity($zoneId,$getField=array('*')){
        //获取商圈内的店铺
        $shopCode = M('shop')->where(array('bank_id'=>$zoneId,'status'=>1))->getField('shopCode,shopName');
        if ($shopCode) {
            $activityList = M('activity')->field($getField)->where(array('status' => 1))->where(array('shopCode' => array('in', array_keys($shopCode))))->select();
            if($activityList){
                return array('active'=>$activityList,'shopData'=>$shopCode,'code'=>200);
            }else{
         ...
         if($asData['active']){
                ...
                foreach($asData['active'] as $v){
                    ...
                    if(strtotime($v['endTime'])>time()){
                        foreach($asData['shopData'] as $k1 => $v1){
                            if($v['shopCode'] == $k1 ){
                                $v['shopName'] = $v1;
                            }
                        }
    ...
    //获取已过期活动
    function getEndActivity($zoneId){
        $asData = $this->activity($zoneId);
        if($asData['active']){
            $list = array();
            foreach($asData['active'] as $v){
                //获取未过期活动
                if(strtotime($v['endTime'])<time()){
                    foreach($asData['shopCode'] as $k1 => $v1){
                        if($v['shopCode'] == $k1){
                            $v['shopName'] = $v1;
                        }
                    }
    ...
    function getOnActivity($userCode){
        $activityCode = M('useractivity')->where(array('userCode' => $userCode))->field('activityCode')->select();
        $onActivity = array();
        foreach($activityCode as $k =>$v){
            $onActivity[] = M('activity')->where(array('activityCode'=>array('eq',$v['activityCode']),'status'=>array('eq',1)))->find();
        }
        $onActivity = array();
        if(!empty($activityCode)){
            foreach($activityCode as $v){
    ...
    function getActivityComment($activityCode){
        $activityData = M('activity')->where(array('activityCode' => $activityCode,'status'=>1))->find();
        if($activityData){
            return $activityData;
        }else{
            return '出现错误';
...
//            $subsidyAmount = $v['platBonus'] + $v['firstDeduction'];
     ...
     * @return array
     */
    public function applyEntry($shopCode, $shopName,$bank_id, $tel, $startTime, $endTime, $street, $mobileNbr) {
        $condition = array();
        $data = array(
            'shopCode' => $shopCode,
            'shopName' => $shopName,
...
//            'brandId' => $brandId
    ...
    public function activate($mobileNbr, $validateCode, $password,$bank_id) {
        ...
        $shopMdl = new ShopModel();
        $ret = $shopMdl->activate($mobileNbr, $validateCode, $password,$bank_id);
        return $ret;
    }
