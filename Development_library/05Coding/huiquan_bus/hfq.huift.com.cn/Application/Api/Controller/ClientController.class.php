...
//            'orderStatus' => C('FOOD_ORDER_STATUS.ORDERED'),
     ...
     * @return array
     */
    public function addCouponOrder($userCode, $shopCode, $batchCouponCode, $couponNbr, $platBonus, $shopBonus,$activityCode) {
        ...
        $orderActivityCode = '';
        $amountDiscountId = '';
        $eachCouponPayPrice = $batchCouponInfo['payPrice'];
        //参加活动获得优惠价格，未参加活动获得原始价格
        if($activityCode && !empty($activityCode)){
            $activityMdl = new ActivityModel();
            $amountRes = $activityMdl->getTruePrice($activityCode,$batchCouponCode,$batchCouponInfo['payPrice'],$couponNbr);
            $orderAmount = $amountRes['orderAmount'];
            $amountDiscountId = $amountRes['amountDiscountId'];
            $eachCouponPayPrice = $amountRes['eachPrice'];
            //c如果参加活动，将活动码添加到订单中
            $orderActivityCode = $activityCode;
        }
        ...
        $orderInfo = array(
            'shopCode' => $shopCode,
            'clientCode' => $userCode,
            'orderAmount' => $orderAmount,//$batchCouponInfo['payPrice'] * $couponNbr,
            'activityCode' => $orderActivityCode,
        );
        $consumeOrderMdl = new ConsumeOrderModel();
       ...
       // 保存该笔订单买了哪些券
            for($i = 0; $i < $couponNbr; $i++) {
                $orderCouponInfo = array(
                    ...
                    'costMoney' => $eachCouponPayPrice, //付款时的优惠券单价
                );
                // 保存优惠券
            ...
            //往amountdiscount表中添加订单号
            if($amountDiscountId && !empty($amountDiscountId))
                M('amountdiscount')->where(array('id' => $amountDiscountId))->setField('orderCode', $orderCode);
            // 减少优惠券的剩余数量
...
8、本规则最终解释权归惠圈所有'
     ...
     * @return array $moduleList
     */
    public function getClientHomePage($city,$zoneId) {
//        $clientAMMdl = new ClientActModuleModel();
//        $moduleList = $clientAMMdl->listClientActModule(array(), $this->getPager(1));
//        foreach($moduleList as $index => &$module) {
...
//        }
        ...
        $shopTypeList = $cityShopTypeMdl->listCityShopType2($cityInfo['id'],$zoneId);
        return array(
//            'moduleList' => $moduleList,
     ...
     * @return array $ret
     */
    public function updateUserInfo($mobileNbr,$userCode, $updateInfo) {
		$preg_realName_name='/^[\x{4e00}-\x{9fa5}]{2,7}$/u';
		$preg_nick_name='/^[\x{4e00}-\x{9fa5}\w]{1,15}$/u';
		if(!empty($updateInfo['realName'])){
			if(!preg_match($preg_realName_name,$updateInfo['realName'])) {return  '真实名字不合法';}
		}
		if(!empty($updateInfo['nickName'])){
			if(!preg_match($preg_nick_name,$updateInfo['nickName'])) {return  '昵称不合法';}
		}
    ...
    public function getUserInfo($userCode) {
       ...
       if(strstr($userInfo['avatarUrl'],'http')){
           $userInfo['avatarUrl'] =   str_replace('http','https',$userInfo['avatarUrl']); //将http替换成https
       }
     ...
     * @return array
     */
    public function listSearchWords($city,$zoneId) {
        ...
        // 获得行业列表
        $cityShopTypeMdl = new CityShopTypeModel();
        $cityShopTypeList = $cityShopTypeMdl->listCityShopType2($cityId['id'],$zoneId);
        $type = array(
            array(
                'queryName' => '所有',
                'value' => 0,
     ...
     */
    public function searchShop($searchWord, $type, $longitude, $latitude, $userCode, $page, $city, $moduleValue = 0, $content = '', $order = 0, $filter = 0,$queryName,$zoneId) {
        ...
        if($moduleValue == \IndexModule::SHOP_TRADING_AREA){ //商圈
            ...
            $condition['Shop.district'] = $queryName;
        }
        if(isset($infoArr)){
            $shopCodeArr = $infoArr;
            if(isset($relList)){
        ...
        $shopList = $shopMdl->searchShop($condition, '', $this->getPager($page), $joinTableArr, $order,$zoneId);

        $shopCount = $shopMdl->countSearchShop($condition, $joinTableArr,$zoneId);
        return array(
    ...
    public function getShopInfo($shopCode, $userCode = '',$longitude,$latitude) {
        ...
        $shopCoupon = $batchCouponMdl->listAvailabelCoupon($shopCode,
            array('batchCouponCode','couponName','totalVolume','remaining','insteadPrice','discountPercent',
                'availablePrice','endTakingTime','createTime','couponType','function','batchNbr','startUsingTime',
                'expireTime','dayStartUsingTime','dayEndUsingTime','remark','city','shopName','BatchCoupon.payPrice',
                'BatchCoupon.shopCode','sqrt(power(Shop.longitude-'.$longitude.',2) + power(Shop.latitude-'.$latitude.', 2))' => 'distance','Shop.logoUrl','CollectCoupon.isCollect','CollectCoupon.isGet',
            ), $userCode,$longitude,$latitude);//添加经纬度
        foreach($shopCoupon as $k => &$v) {
            unset($shopCoupon[$k]['takenCount']);
            unset($shopCoupon[$k]['takenPercent']);
            //判断是否有券正在进行活动
            $activityCode =  M('couponactivity')->where(array('batchCouponCode' => $v['batchCouponCode']))->find();

            if($activityCode['activityCode']){
              //判断活动的时效性和有效性
                $activityMdl = D('Activity');
                $onAct = $activityMdl->isTrueActivity($activityCode['activityCode']);

                if($onAct){
                    $price = $activityMdl->$activityCode['function']($v['payPrice']*100,$couponNum=1,$type=true);
                    $v['oldPrice'] = $v['payPrice'];
                    $v['payPrice'] = $price['newPrice'];
                    $v['activityCode'] = $activityCode['activityCode'];
                }
            }
        }
        // 添加用户浏览商家详情记录
...
//            $v['photoList'] = $photoList;
     ...
     */
    public function listCoupon($couponType, $searchWord, $longitude, $latitude, $page, $city, $userCode = '',$zoneId) {
        $batchCouponMdl = new BatchCouponModel();

        $couponList = $batchCouponMdl->listCoupon2($couponType, $searchWord, $longitude, $latitude, $this->getPager($page), $city, $userCode,$zoneId);
	    $couponCoupon = $batchCouponMdl->countCoupon2($couponType, $searchWord, $longitude, $latitude, $city,$userCode,$zoneId);
       return array(
    ...
    public function getMyAvailableCoupon($userCode, $shopCode, $status, $page, $longitude, $latitude,$city,$zoneId) {
        $userCouponMdl = new UserCouponModel();
        $userCouponList = $userCouponMdl->getMyAvailableCoupon($userCode, $shopCode, $status, $this->getPager($page), $longitude, $latitude,$city,$zoneId);
        $userCouponCount = $userCouponMdl->countMyAvailableCoupon($userCode, $shopCode, $status,$zoneId);
        return array(
    ...
    public function getUserCouponInfo($userCouponCode) {
        ...
        $userCouponInfo[0]['payPrice'] = ($userCouponInfo[0]['payPrice']/100); //分转元
...
//        var_dump($userCouponList);
     ...
     * @return array $ret
     */
    public function bankcardPay($userCode, $shopCode, $orderAmount, $userCouponCode, $platBonus, $shopBonus,$noDiscountPrice) {

		//根据shopCode查询到shopName
		$shopModel=D('Shop');
		$arrShopName=$shopModel->field('shopName')->where("shopCode='$shopCode'")->find();
		...
		$realPayWeiXin=$ret['realPay'];
		$consumeCode=$ret['consumeCode'];
		$arrOrderCode=$userConsumeMdl->field('orderCode')->where("consumeCode='$consumeCode'")->find();
		$orderCode=$arrOrderCode['orderCode'];
		$ConsumeOrder=D('ConsumeOrder');
        $data['realPayWeiXin']=$realPayWeiXin*100;
	    $result =	$ConsumeOrder->where("orderCode='$orderCode'")->save($data);
        return $ret;
    }
    ...
    public function bankcardPayForAndroid($userCode, $shopCode, $orderAmount, $batchCouponCode, $nbrCoupon, $platBonus, $shopBonus, $noDiscountPrice) {
        ...
        $ConsumeOrder=D('ConsumeOrder');
        $data['realPayWeiXin']=$realPayWeiXin*100;
        $ConsumeOrder->where("orderCode='$orderCode'")->save($data);
        return $ret;
    }
    ...
    /**
     * 获取已经开通的城市(乔本亮修改Saas版本)
     * @return array $cityList
     */
    public function zonelistOpenCity($zoneId){
        $dist_bank_model = new DistrictBankIdModel();
        $cityList = $dist_bank_model->zonelistOpenCity($zoneId);
        return $cityList;
    }
	 ...
	 */
	 public function listOpenCityH5($parentId){
        $districtMdl = new DistrictModel();
        $cityList = $districtMdl->listOpenCityH5($parentId);
        return $cityList;
    }
...
//        var_dump($calMdl->getNewestClientAppVersion());
    ...
    public function getUserOrderList($userCode,  $isFinish, $page){
        ...
        if($isFinish == 1) { // 支付成功
            $condition['UserConsume.status'] = $condition['ConsumeOrder.status'] =
                array("IN",array(\Consts::PAY_STATUS_CAN_NOT_PAY,\Consts::PAY_STATUS_REFUNDING,\Consts::PAY_STATUS_REFUNDED,\Consts::PAY_STATUS_PART_REFUNDED,\Consts::PAY_STATUS_PAYED));
        } else { // 支付失败和未支付
            $condition['UserConsume.status'] = $condition['ConsumeOrder.status'] =
                array('IN', array(\Consts::PAY_STATUS_UNPAYED, \Consts::PAY_STATUS_FAIL, \Consts::PAY_STATUS_PAYING,\Consts::PAY_STATUS_CANCELED ));
        }
