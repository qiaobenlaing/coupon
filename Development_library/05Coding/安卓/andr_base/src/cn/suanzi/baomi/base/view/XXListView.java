...
public class XXListView extends ListView implements OnScrollListener {
	...
	private static final String TAG = XXListView.class.getSimpleName();
...
//						mHeaderViewHeight = mHeaderViewContent.getHeight();
	...
	public void stopRefresh() {
		if (mPullRefreshing == true) {
			...
			Log.d("TAG", "跟新的时间=====111"+System.currentTimeMillis());
			resetHeaderHeight(0);
		}
	}
	...
	private void refreshUpdatedAtValue() {
		...
		Log.d("TAG", "更新的时间====取值"+lastUpdateTime);
		long currentTime = System.currentTimeMillis();
		long timePassed = currentTime - lastUpdateTime;
		Log.d("TAG", "更新的时间====和当前时间比较："+timePassed);
		long timeIntoFormat;
	...
	private void resetHeaderHeight(int scollHeight) {
		...
		// refreshUpdatedAtValue();
		Log.d(TAG, "scrollHeight >>> "+height + " >>> " + (finalHeight - height));
...
//		setSelection(mTotalItemCount - 1); // scroll to bottom
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
		...
		if (mScrollListener != null) {
			...
			Log.d(TAG, "下拉   >>>>> 刷新  onScroll >>> 5");
			if(firstVisibleItem+visibleItemCount>=totalItemCount){
				pullable=true;
			}else{
				pullable=false;
				Log.d(TAG, "下拉   >>>>> 刷新  onScroll >>> 7");
			}
		}
	}
