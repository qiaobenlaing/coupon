 ...
 * The JSON-RPC 2.0 messages are dispatched by means of HTTP(S) POST. This class
 * is thread-safe.
 * 
 * <p>
 * The client-session class has a number of {@link JSONRPC2SessionOptions
...
public class JSONRPC2Session {
	...
	/**
	 * Creates a new client session to a JSON-RPC 2.0 server at the specified
	 * URL. Uses a default {@link JSONRPC2SessionOptions} instance.
	 * 
	 * @param url
	 *            The server URL, e.g. "http://jsonrpc.example.com:8080". Must
	 *            not be {@code null}.
	 */
	 ...
	 * 
	 * @param options
	 *            The client session options, must not be {@code null}.
	 */
	 ...
	 * connection after the {@link JSONRPC2SessionOptions session options} are
	 * applied and before the connection is established.
	 * 
	 * <p>
	 * This method may be used to set custom HTTP request headers, timeouts or
	 * other properties.
	 * 
	 * @since 1.5
	 * 
	 * @param connectionConfigurator
	 *            A custom HTTP URL connection configurator, {@code null} to
	 *            remove a previously set one.
	 */
	public void setConnectionConfigurator(
	 ...
	 * 
	 * <p>
	 * You can use the {@link RawResponseInspector} interface to retrieve the
	 * unparsed response content and headers.
	 * 
	 * @since 1.6
	 * 
	 * @param responseInspector
	 *            An optional inspector for the raw HTTP responses, {@code null}
	 *            to remove a previously set one.
	 */
	public void setRawResponseInspector(
			final RawResponseInspector responseInspector) {
	 ...
	 * 
	 * @return The HTTP cookies, or empty list if none were set by the server or
	 *         cookies are not {@link JSONRPC2SessionOptions#acceptCookies
	 *         accepted}.
	 */
	 ...
	 * 
	 * @param con
	 *            The URL connection which must be open.
	 * 
	...
	/**
	 * Creates and configures a new URL connection to the JSON-RPC 2.0 server
	 * endpoint according to the session settings.
	 * 
	 * @return The URL connection, configured and ready for output (HTTP POST).
	 * 
	 * @throws JSONRPC2SessionException
	 *             If the URL connection couldn't be created or configured.
	 */
	private URLConnection createURLConnection() throws JSONRPC2SessionException {
		...
		try {
			...
			throw new JSONRPC2SessionException("Network exception: "
					+ e.getMessage(),
		...
		if (con instanceof HttpsURLConnection && options.trustsAllCerts()) {
			...
			if (trustAllSocketFactory == null)
				throw new JSONRPC2SessionException(
						"Couldn't obtain trust-all SSL socket factory");

			((HttpsURLConnection) con)
					.setSSLSocketFactory(trustAllSocketFactory);
		}
	 ...
	 * 
	 * @param con
	 *            The URL connection. Must be in HTTP POST mode. Must not be
	 *            {@code null}.
	 * @param data
	 *            The string data to post. Must not be {@code null}.
	 * 
	 * @throws JSONRPC2SessionException
	...
	/**
	 * Reads the raw response from an URL connection (after HTTP POST). Invokes
	 * the {@link RawResponseInspector} if configured and stores any cookies
	 * {@link JSONRPC2SessionOptions#storeCookies if required}.
	 * 
	 * @param con
	 *            The URL connection. It should contain ready data for
	...
	private RawResponse readRawResponse(final URLConnection con)
		...
		if (options.acceptCookies()) {
			...
			try {
				cookieManager.put(con.getURL().toURI(),
				...
				throw new JSONRPC2SessionException("I/O exception: "
						+ e.getMessage(),
						JSONRPC2SessionException.NETWORK_EXCEPTION, e);
			}
		}
	 ...
	 * 
	 * @param request
	 *            The JSON-RPC 2.0 request to send. Must not be {@code null}.
	 * 
	 * @return The JSON-RPC 2.0 response returned by the server.
	 * 
	 * @throws JSONRPC2SessionException
	 *             On a network error, unexpected HTTP response content type or
	...
	public JSONRPC2Response send(final JSONRPC2Request request)
		...
		Log.i("RawResponse Content", rawResponse.getContent());
		// Log.e("RawResponse ContentEncoding",rawResponse.getContentEncoding());
		Log.i("RawResponse ContentType", rawResponse.getContentType());
		Log.i("RawResponse StatusMessage", rawResponse.getStatusMessage());
		Log.i("RawResponse HeaderFields", rawResponse.getHeaderFields()
				.toString());
		Log.i("RawResponse StatusCode", rawResponse.getStatusCode() + "");
		...
		if (!options.isAllowedResponseContentType(contentType)) {
			...
			if (contentType == null)
				...
				msg = "Unexpected \"" + contentType
						+ "\" content type of the HTTP response";

			throw new JSONRPC2SessionException(msg,
		...
		try {
			response = JSONRPC2Response.parse(rawResponse.getContent(),
					options.preservesParseOrder(), options.ignoresVersion(),
					options.parsesNonStdAttributes());

		} catch (JSONRPC2ParseException e) {

			throw new JSONRPC2SessionException("Invalid JSON-RPC 2.0 response",
					JSONRPC2SessionException.BAD_RESPONSE, e);
		}
		...
		if (reqID != null && resID != null
				&& reqID.toString().equals(resID.toString())) {
			// ok
		} else if (reqID == null && resID == null) {
			// ok
		} else if (!response.indicatesSuccess()
				&& (response.getError().getCode() == -32700
						|| response.getError().getCode() == -32600 || response
						.getError().getCode() == -32603)) {
			// ok
		} else {
			throw new JSONRPC2SessionException(
	...
	/**
	 * Sends a JSON-RPC 2.0 notification using HTTP POST. Note that contrary to
	 * requests, notifications produce no server response.
	 * 
	 * @param notification
	 *            The JSON-RPC 2.0 notification to send. Must not be
	 *            {@code null}.
	 * 
	 * @throws JSONRPC2SessionException
	 *             On a network error.
