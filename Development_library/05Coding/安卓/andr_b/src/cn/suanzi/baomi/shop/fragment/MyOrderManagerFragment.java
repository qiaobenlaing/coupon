...
import cn.suanzi.baomi.shop.adapter.MyOrderAdapter;
import cn.suanzi.baomi.shop.model.CountOrderByTypeTask;
import cn.suanzi.baomi.shop.model.GetOrderListForBTask;
...
public class MyOrderManagerFragment extends Fragment implements IXListViewListener{
	private static final String TAG = MyOrderManagerFragment.class.getSimpleName();
	/** 扫描条形码的标志*/
		...
		new GetOrderListForBTask(getActivity(), new GetOrderListForBTask.Callback() {
			...
			public void getResult(JSONObject result) {
				...
				if (result == null) {
					...
					Log.d(TAG, "不是餐饮的订单：" + result.toString());
		...
		new GetShopOrderTypeTask(getActivity(), new GetShopOrderTypeTask.Callback() {
			...
			public void getResult(JSONObject result) {
				if (result == null) {
					...
					Log.d(TAG, "是否是餐饮：" + result.toString());
