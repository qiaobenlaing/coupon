...
public class HomeFragment extends BaseFragment implements IXListViewListener {
		...
		new GetHomeInfoTask(getMyActivity(), new GetHomeInfoTask.Callback() {
			...
			public void getResult(JSONArray result) {
				...
				if (null != result) {
					...
					List<HomePage> homePages = new Gson().fromJson(result.toString(), new TypeToken<List<HomePage>>() {}.getType());
					setHomeView(homePages, ONLINE); // 设置布局
				} else {
					if (mFlagArea) {
						DialogUtils.showDialogSingle(getActivity(), Util.getString(R.string.no_city_open),
								R.string.cue, R.string.ok, null);
					} else {
						// TODO 不用管
					}
				}
			...
			for (int i = 0; i < homePages.size(); i++) {
				...
				if (homePage.getModuleValue() == CustConst.Home.SCROLL_VALUE) { // 滚屏
					...
					try {
						...
						// 获取布局文件的id
						int id = CustUtil.getResourceId(getActivity(), actLayoutName, "layout", "cn.suanzi.baomi.cust");
	...
	public void setShopAct(List<HomeTemplate> subList, final String moduleValue) {
		...
		for (int i = 1; i < actCount + 1; i++) {
			...
			// 根据名称获取id
			int widgetId = CustUtil.getWidgetId(getMyActivity(), rl_id_name, "cn.suanzi.baomi.cust");
			// 获取RelativeLayout
...
//				getNestBaidu();
		...
		if (null != home) {
			...
			if (strIsEmpty(home.getHomeResultString())) {
				try {
					...
					List<HomePage> homePages = new Gson().fromJson(homePage.toString(),new TypeToken<List<HomePage>>() {}.getType());
			...
			for (int i = 0; i < actList.size(); i++) {
				try {
					...
					if (i == 0) {
						...
						paramsScroll.height = (int) (Util.getWindowWidthAndHeight(getMyActivity())[0] / homeTemplate
								.getImgRate());
	...
	private Runnable mSrollRunnable = new Runnable() {
		...
		@Override
		public void run() {
			if (mScrollPics == null || mScrollPics.length == 0) { return;// TODO
			}
			/*** 更新界面 **/
						...
						if (flag) {
							...
							if (firstRun) {
								...
								View view = LayoutInflater.from(getMyActivity()).inflate(
										R.layout.popupwindow_nogpscity, null);
