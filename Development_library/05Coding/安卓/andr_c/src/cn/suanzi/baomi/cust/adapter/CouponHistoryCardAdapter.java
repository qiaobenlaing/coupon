 ...
 * @author yanfang.li
 */
public class CouponHistoryCardAdapter extends CommonListViewAdapter<BatchCoupon> {

	private final static String TAG = CouponEffectCardAdapter.class.getSimpleName();

	public CouponHistoryCardAdapter(Activity activity, List<BatchCoupon> datas) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		CommenViewHolder holder = CommenViewHolder.get(mActivity, convertView, parent, R.layout.item_couponhistory,
				position);
		final BatchCoupon coupon = mDatas.get(position);
		// 商家Logo
		ImageView ivCardImage = ((ImageView) holder.getView(R.id.iv_couponpic));
		// 商家图片显示图片
		Util.showImage(mActivity, coupon.getLogoUrl(), ivCardImage);
		// 店铺名称
