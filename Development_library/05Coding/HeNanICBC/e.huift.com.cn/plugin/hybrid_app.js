...
function callBackAddress(param) {
    var addressUrl = "defaultAdress.do?addressParams="+param;
    $.ajax({
		type : "POST",
		url : addressUrl,
		dataType : 'json',
		...
		success : function(msg) {
			if(msg.id != null && msg.id != ""){
				$("#addressId").text(msg.id);
				$("#cust_id").text(msg.cust_id);
				$("#name").text(msg.name);
				$("#phone").text(msg.phone);
				$("#address").text(msg.address);
				$("#provinceName").text(msg.provinceName);
				$("#cityName").text(msg.cityName);
				$("#districtName").text(msg.districtName);
				$("#updateTimeStr").text(msg.updateTimeStr);
				$("#isDefault").text(msg.isDefault);
				$("#addrSuccess").text("获取默认地址成功!");
			}else{
				$("#addrSuccess").text("获取默认地址失败!");
			}
		},
		error : function(XMLHttpRequest, textStatus, errorThrown) {
			alert($.parseXML(XMLHttpRequest) + "," + textStatus + ","
...
function thirdScanCallback(param) {
	 if(param != null && param != ""){
		 var code = JSON.parse(param);
	   		alert(code.QRCodeResult);
	    }else{
	   		alert("扫码失败");
	    }
}
