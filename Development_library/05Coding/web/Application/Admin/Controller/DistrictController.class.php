...
class DistrictController extends AdminBaseController {
    ...
    public function listCity() {
        ...
        $cityList = $districtMdl->listCity(I('get.'), $this->pager);
        $cityCount = $districtMdl->countCity(I('get.'));
        if(I('get.name') == '浙江省') {
            $yiwu = $districtMdl->getCityInfo(array('name' => '义乌市', 'isOpen' => I('get.isOpen')), array('*'));
            if($yiwu) {
                $cityList[] = $yiwu;
                $cityCount += 1;
            }
    ...
    public function changeStatus() {
        ...
        $res = $districtMdl->changeCityStatus($id, $isOpen);
        if ($res === true) {
            $this->ajaxSucc('操作成功!', $resp);
        } else {
            $this->ajaxError($res);
        }
    }
