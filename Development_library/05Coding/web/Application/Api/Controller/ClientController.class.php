...
//        }
        $shopTypeList = S("shopTypeList".$city);
        if(!$shopTypeList){
            ...
            S("shopTypeList".$city,json_encode($shopTypeList));
        }
        return array(
...
//        var_dump($result['type']);
    ...
    public function searchShop($searchWord, $type, $longitude, $latitude, $userCode, $page, $city, $moduleValue = 0, $content = '', $order = 0, $filter = 0,$queryName) {
        ...
        $shopList = S("shopInfoList".$condition.$page.$order);

        if(!$shopList){

            $shopList = $shopMdl->searchShop($condition, '', $this->getPager($page), $joinTableArr, $order);

            S("shopInfoList".$condition.$page.$order,json_encode($shopList));
        }
    ...
    public function getShopInfo($shopCode, $userCode = '',$longitude,$latitude) {
        ...
        $shopCoupon = $batchCouponMdl->listAvailabelCoupon($shopCode,
            array('batchCouponCode','couponName','totalVolume','remaining','insteadPrice','discountPercent',
                'availablePrice','endTakingTime','createTime','couponType','function','batchNbr','startUsingTime','expireTime',
                'dayStartUsingTime','dayEndUsingTime','remark','city','shopName','BatchCoupon.payPrice','BatchCoupon.shopCode',
                'sqrt(power(Shop.longitude-'.$longitude.',2) + power(Shop.latitude-'.$latitude.', 2))' => 'distance','Shop.logoUrl',
                'CollectCoupon.isCollect','CollectCoupon.isGet',
...
//        var_dump($userCouponList);
    ...
    public function listOpenCity(){
        ...
        $cityList = S("listOpencity");
        if(!$cityList){
            $districtMdl = new DistrictModel();
            $cityList = $districtMdl->listOpenCity();
            S("listOpencity",json_encode($cityList));
        }
        return $cityList;
    }
...
//        var_dump($calMdl->getNewestClientAppVersion());
    ...
    public function getUserOrderList($userCode,  $isFinish, $page){
        ...
        if($isFinish == 1) { // 支付成功
            ...
            $condition['UserConsume.status'] = $condition['ConsumeOrder.status'] = array('IN', array(\Consts::PAY_STATUS_UNPAYED, \Consts::PAY_STATUS_FAIL, \Consts::PAY_STATUS_PAYING,\Consts::PAY_STATUS_CANCELED ));
        }
