...
// 获得用户在该商户的下单并且支付成功过的次数
     ...
     */
	  public function bankcardPayConfirm2($consumeCode, $isUseFirstDeduction = 1, $payChanel = \Consts::PAY_CHANEL_NULL) {
       //根据消费编码查询查询orderCode
	 //  $arrorderCode=$this->where("consumeCode='$consumeCode'")->find();

	//  $orderCode=$arrorderCode['orderCode'];

	   //根据订单号查询优惠券编码
	//   $OrderCouponModel=D('OrderCoupon');
	//   $arrbatchCouponCode=$OrderCouponModel->where("orderCode='$orderCode'")->find();

	//   $batchCouponCode=$arrbatchCouponCode['batchCouponCode'];

	//   $userCode=$arrbatchCouponCode['userCode'];
	...
	/*	$arr=array(
		...
		if($isGetCount>$remainingCount['remaining']){
			...
			$CollectCouponModel->isGetCount=$remainingCount['remaining'];
          $a=  $CollectCouponModel->where("userCode='$userCode' and collectCouponCode='$batchCouponCode'")->save();

		}else{
			  //在CollectCoupon修改客户剩余领取数量
			$CollectCouponModel->isGetCount=$isGetCount;
			$CollectCouponModel->where("userCode='$userCode' and collectCouponCode='$batchCouponCode'")->save();
		}
        ...
        if($pubArr['retcode'] == C('ICBC_PAY_SUCCESS')) {
            ...
            // 更新用户支付状态为已支付和支付完成时间
            $retUserConsume = $this->updateConsumeStatus(array('consumeCode' => $consumeCode), array('status' => \Consts::PAY_STATUS_PAYED, 'payedTime' => date('Y-m-d H:i:s')));
            if($incConsumeCountRet == true && $retOfUpdatePoint == true && $upOrderStatusRet == true && $retUserConsume == true) {
                M()->commit();
// 获得用户在该商户的下单并且支付成功过的次数
                $orderCount = $consumeOrderMdl->countOrder(array('clientCode' => $userCode, 'shopCode' => $shopCode, 'ConsumeOrder.status' => array('IN', array(\Consts::PAY_STATUS_PAYED, \Consts::PAY_STATUS_REFUNDING, \Consts::PAY_STATUS_REFUNDED, \Consts::PAY_STATUS_PART_REFUNDED))));
                // 增加商家回头客人数
    ...
    public function bankCardPay($orderCode, $userCouponCode, $platBonus, $shopBonus, $noDiscountPrice = 0.0, $isUseUserCard = 1, $isUseBankCard = 1) {
        ...
        $userConsumeMdl = new UserConsumeModel();
        // 得到是否为首单
        $isFirst = $userConsumeMdl->isFirst($orderInfo['clientCode']);
        $firstDeduction = 0;
        ...
        $bankCardDeduction = 0; // 设置商家工银支付折扣金额为0分
        ...
        $userCardMdl = new UserCardModel();
        // 获得顾客在某商店拥有的最高级的会员卡的信息
        $userCardInfo = $userCardMdl->getBestUserCard($userCode, $orderInfo['shopCode']);
        ...
        if($userCouponInfo && $userCouponInfo['couponType'] == C('COUPON_TYPE.N_PURCHASE')) { // N元购型优惠券的使用
            ...
            if($userCardInfo) {
                ...
                $cardActionLogInfo = array(
                    ...
                    'consumeAmount' => $realPay, // 消费金额，单位：分
            ...
            if($shopBonus > 0) {
                // 使用商家红包
                $shopBonusRet = $bsMdl->reduceBonusStatistics($userCode, $orderInfo['shopCode'], $shopBonus);
                if($shopBonusRet['code'] == C('SUCCESS')) {
                    $shopBonus = intval($shopBonus);
                    $realPay = $realPay - ceil($shopBonus);
                } else {
                    return $shopBonusRet;
                }
            ...
            $shopMdl = new ShopModel();
            // 获得商家信息（商户对工行卡的折扣，商户对在线支付的抵扣金额上限（单位：折），是否受理银行卡支付）
            $shopInfo = $shopMdl->getShopInfo($orderInfo['shopCode'], array('onlinePaymentDiscount', 'onlinePaymentDiscountUpperLimit', 'isAcceptBankCard'));
            // 判断是否使用 商家对银行卡的折扣。买券订单，买活动码订单不享有工银折扣
            if($shopInfo['onlinePaymentDiscount'] < 10 && $shopInfo['isAcceptBankCard'] == true && ! in_array($orderInfo['orderType'], array(\Consts::ORDER_TYPE_COUPON, \Consts::ORDER_TYPE_ACT)) && $isUseBankCard == \Consts::YES) {
                $bankCardDeduction = $realPay - $realPay * $shopInfo['onlinePaymentDiscount'] / 10; // 工银折扣抵扣金额，单位：分
                if($shopInfo['onlinePaymentDiscountUpperLimit'] != 0) {
                    // 判断抵扣额是否大于商店设置的上限
                    if($bankCardDeduction > $shopInfo['onlinePaymentDiscountUpperLimit'] * \Consts::HUNDRED) {
                        $bankCardDeduction = $shopInfo['onlinePaymentDiscountUpperLimit'] * \Consts::HUNDRED;
                    }
                }
                $bankCardDeduction = ceil($bankCardDeduction);
                $realPay = $realPay - $bankCardDeduction;
            }
        }
        ...
        if($canDiscountAmount > 1000) {

        } elseif($canDiscountAmount > 0 && $canDiscountAmount <= 1000) {
        ...
        // 获得在线支付的商户号
        $shopMdl = new ShopModel();
        $shopInfo = $shopMdl->getShopInfo($orderInfo['shopCode'], array('icbcShopCode', 'hqIcbcShopNbr'));
        // 如果商户号是10位，则走的是中间商户支付流程，否则不是
        ...
        $consumeLog = array(
            ...
            'usedCardCode' => $userCardInfo['cardCode'] ? $userCardInfo['cardCode'] : '',
...
//        echo $this->getLastSql();
     ...
     * @return array
     */
    public function zeroPay2($userCode, $shopCode, $userCouponCode, $appType){
        $rules = array(
            array('userCode', 'require', C('USER.USER_CODE_EMPTY'), 1),
            array('shopCode', 'require', C('SHOP.SHOP_CODE_EMPTY'), 1),
...
//                'identityCode' => substr(md5(uniqid(mt_rand(), true)), 0, 12),
    ...
    public function filterUserConsumeWhere(&$where) {
        ...
        if($where['OrderStatus']){
            $where['OrderCoupon.status'] = $where['OrderStatus'];
            unset($where['OrderStatus']);
        }
    ...
    public function listUserConsume($filterData, $page, $sortType = '', $field = array()) {
        $where = $this->filterWhere(
            $filterData,
            array('shopName' => 'like', 'mobileNbr' => 'like', 'orderNbr' => 'like','function'=>'like'),
            $page);
        $where = $this->filterUserConsumeWhere($where);
        if(empty($sortType)) {
        ...
        if(empty($field)) {
            $field = array('orderTime', 'orderNbr', 'orderAmount','BatchCoupon.function','OrderCoupon.status'=>'userOrderStatus', 'shopName', 'Shop.logoUrl', 'Shop.city', 'hqIcbcShopNbr', 'nickName', 'User.avatarUrl', 'User.mobileNbr' => 'userMobileNbr', 'deduction', 'realPay', 'UserConsume.status' => 'userConsumeStatus', 'isCard', 'consumeCode', 'couponUsed', 'bankCardDeduction', 'firstDeduction', 'payType', 'usedCardCode', 'usedUserCouponCode', 'orderType', 'orderStatus', 'ConsumeOrder.orderCode', 'userCouponCode', 'orderConfirm', 'consumeTime', 'couponDeduction', 'cardDeduction', 'UserConsume.realPay', 'ConsumeOrder.orderTime', 'UserConsume.payedTime');
        }
