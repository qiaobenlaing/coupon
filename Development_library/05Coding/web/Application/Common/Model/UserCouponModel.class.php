...
BatchCoupon.validityPeriod)';
    ...
    public function isUserCanGrabTheCoupon($userCode, $batchCouponCode) {
        $count = $this->where(array('userCode' => $userCode, 'batchCouponCode' => $batchCouponCode))->count('userCouponCode');
        $batchCouponMdl = new BatchCouponModel();
...
//        $serialNbr = sprintf('%05d', intval(substr($maxUserCouponNbr, -5)) + 1);
     ...
     * @return array
     */
    public function grabCoupon2($batchCouponCode, $userCode, $sharedLvl,$from,$type = 0) {
        // 诺亚方舟影院9.9N元购和19.9N元购只能享受其中一种。抢这两种券时加以判断
        ...
        if($ret !== false) {
            ...
            if($isGetCount>$remainingCount['remaining']){
                ...
                $CollectCouponModel->isGetCount=0;
                $CollectCouponModel->where("userCode='$userCode' and collectCouponCode='$batchCouponCode'")->save();

                $isGetCount=$remainingCount['remaining']-1;
                if($isGetCount<0){
                    return array("code"=>80221);
                }
            }else{
...
//                $v['dayEndUsingTime'] = substr($v['dayEndUsingTime'], 0, 5);
     ...
     * @return array
     */
    public function countMyAvailableCoupon($userCode, $shopCode, $status){
