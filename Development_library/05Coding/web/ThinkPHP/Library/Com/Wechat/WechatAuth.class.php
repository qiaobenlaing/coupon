...
class WechatAuth {
    ...
    /**
     * 获取授权用户信息
     * @param  string $tokenNOpenId 包含 $tokenNOpenId['access_token'] 和 $tokenNOpenId['openid']
     * @param  string $lang  指定的语言
     * @return array         用户信息数据，具体参见微信文档
     */
    public function getUserInfo($tokenNOpenId, $lang = 'zh_CN'){
        $query = array(
            'access_token' => $tokenNOpenId['access_token'],
            'openid'       => $tokenNOpenId['openid'],
            'lang'         => $lang,
        );
    ...
    protected static function http($url, $param, $data = '', $method = 'GET'){
		...
		\Think\Log::record('>>>>> ' . json_encode($opts), 'DEBUG');
        ...
        //发生错误，抛出异常
        if($error) {
			\Think\Log::record('<<<<< ' . json_encode($error), 'ERROR');
			throw new \Exception('请求发生错误：' . $error);
		}

		\Think\Log::record('<<<<< ' . json_encode($data), 'DEBUG');
        return  $data;
    }
