...
#import "BMSQ_ShopFilterView.h"
#import "UIImageView+WebCache.h"
@interface BMSQ_ShopFilterView ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation BMSQ_ShopFilterView
...
- (void)setForFilterDic:(NSDictionary *)filterDic{
    NSArray * array = filterDic[@"list"];
    if (array != nil && ![array isKindOfClass:[NSNull class]] && array.count != 0){
        
    for (NSDictionary * newDic in array) {
        BMSQ_filterModel * model = [[BMSQ_filterModel alloc] initWithInforDic:newDic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    static NSString * cell_id = @"ShopFilterView";
    FilterViewCell * cell = [tableView dequeueReusableCellWithIdentifier:cell_id];
    if (!cell) {
        cell = [[FilterViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cell_id];
    }
    BMSQ_filterModel * model = self.dataSource[indexPath.row];
    ...
    if (self.selectTag == indexPath.row) {
        cell.hookeImage.frame = CGRectMake(cell.frame.size.width - 20, cell.frame.size.height / 3, 10, cell.frame.size.height / 3);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self.baseView reloadData];
    BMSQ_filterModel * model = self.dataSource[indexPath.row];
    [self.delegate filterViewTheValue:0 valueNumber:model.value];
