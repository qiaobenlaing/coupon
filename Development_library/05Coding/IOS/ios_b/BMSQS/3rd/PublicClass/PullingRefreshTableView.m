...
- (id)initWithFrame:(CGRect)frame atTop:(BOOL)top {
    ...
    if (self) {
		...
		self.backgroundColor = APP_VIEW_BACKCOLOR;//[UIColor colorWithRed:110.0/255.0 green:52.0/255.0 blue:181.0/255.0 alpha:1.0];

//        self.backgroundColor = [UIColor clearColor];
        UIFont *ft = [UIFont systemFontOfSize:12.f];
        _stateLabel = [[UILabel alloc] init ];
        _stateLabel.font = ft;
        _stateLabel.textColor = [UIColor colorWithRed:65.0/255.0 green:65.0/255.0 blue:65.0/255.0 alpha:1.0];
        ...
        _dateLabel.font = ft;
        _dateLabel.textColor = [UIColor colorWithRed:65.0/255.0 green:65.0/255.0 blue:65.0/255.0 alpha:1.0];
...
- (id)initWithFrame:(CGRect)frame style:(UITableViewStyle)style
{
    ...
    if (self) {
        ...
        float height = 960;
        CGRect rect = CGRectMake(0, 0 - height, APP_VIEW_WIDTH, height);
        _headerView = [[LoadingView alloc] initWithFrame:rect atTop:YES];
        UIImageView* imageView_head = [[UIImageView alloc]initWithFrame:CGRectMake(0, height - 225, APP_VIEW_WIDTH, 225)];
        imageView_head.image = [UIImage imageNamed:@"TableView_head.png"];
        //[_headerView addSubview:imageView_head];
        //[_headerView sendSubviewToBack:imageView_head];
        //[_headerView setBackgroundColor:[UIColor colorWithRed:110.0/255.0 green:52.0/255.0 blue:181.0/255.0 alpha:1.0]];
        [_headerView setBackgroundColor:APP_VIEW_BACKCOLOR];
        [imageView_head release];

        _headerView.atTop = YES;
        [self addSubview:_headerView];
        
        rect = CGRectMake(0, 292.5, APP_VIEW_WIDTH, APP_VIEW_HEIGHT);
        _footerView = [[LoadingView alloc] initWithFrame:rect atTop:NO];
        UIImageView* imageView_foot = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, APP_VIEW_WIDTH, 225)];
        imageView_foot.image = [UIImage imageNamed:@"TableView_foot.png"];
        //[_footerView addSubview:imageView_foot];
        //[_footerView sendSubviewToBack:imageView_foot];
        //[_footerView setBackgroundColor:[UIColor colorWithRed:110.0/255.0 green:52.0/255.0 blue:181.0/255.0 alpha:1.0]];
        [_footerView setBackgroundColor:APP_VIEW_BACKCOLOR];
        //[imageView_foot release];
        _footerView.atTop = NO;
        [self addSubview:_footerView];
        
        [self addObserver:self forKeyPath:@"contentSize" options:NSKeyValueObservingOptionNew context:nil];
        
        //self.backgroundColor = [UIColor colorWithRed:110.0/255.0 green:52.0/255.0 blue:181.0/255.0 alpha:1.0];
...
- (void)setFootOnly:(BOOL)footOnly
{
    _footOnly = footOnly;
    _headerView.hidden = _footOnly;
}
...
- (void)tableViewDidScroll:(UIScrollView *)scrollView {
    ...
    } else if ( yMargin > kPROffsetY - 40){  //footer totally appeared
        if (_footerView.state != kPRStateHitTheEnd) {
            _footerView.state = kPRStatePulling;
        }
    } else if ( yMargin < kPROffsetY + 40 && yMargin > 0) {//footer part appeared
        if (_footerView.state != kPRStateHitTheEnd) {
            _footerView.state = kPRStateNormal;
        }
    }
...
//    } else  if (offset.y + size.height - contentSize.height > kPROffsetY){
        ...
        [UIView animateWithDuration:kPRAnimationDuration animations:^{
            self.contentInset = UIEdgeInsetsMake(0, 0, kPROffsetY + 40, 0);
        }];
