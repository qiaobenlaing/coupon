...
#define GAP 20                       //两个button之前的间距
#define CANCEL_BUTTON_PADDING_TOP 5
#define CONTENT_PADDING_LEFT 22
#define CONTENT_PADDING_TOP 10
#define CONTENT_PADDING_BOTTOM 10
#define BUTTON_HEIGHT 35
#define BUTTON_WIDTH  90
#define CONTAINER_WIDTH 244
#define SHARE_CONTAINER_WIDTH 300
#define SHARE_CONTAINER_HEIGHT 160
...
+(id)alertViewSytle:(SIAlertViewStyle)style{
    
    if (style == SIAlertViewShareCenterStyle) {
        
        SIAlertView *alert=[[SIAlertView alloc] init];
        alert.viewStyle = style;
...
-(id)initWithTowButton{
    
    self=[self initWithTitle:nil andMessage:nil];
    _isTowButton=YES;
...
+(id)passwordAlertView{
    ...
    SIAlertView *alert=[[SIAlertView alloc] initWithTitle:@"" andMessage:@""];
    alert.isPassword = YES;
    alert.isTowButton = YES;
    alert.hasCancelButton = NO;
    alert.passwordTextField = [[UITextField alloc]init];
    alert.passwordTextField.secureTextEntry = YES;
    return alert;
}

+(id)shareCenterAlertView{
...
- (void)show
{
    ...
    if (self.viewStyle == SIAlertViewShareCenterStyle) {
        [self shareCenterLayout];
    }else{
        [self validateLayout];
    }
...
- (void)transitionInCompletion:(void(^)(void))completion
{
    switch (self.transitionStyle) {
        ...
        case SIAlertViewTransitionStyleBounce:
        {
            ...
            animation.values = @[@(0.1), @(1.2), @(0.9), @(1)];
...
//编辑购物车用的样式
-(void)setupEditCartAlertView{
    UIImageView *bgView=[[UIImageView alloc]initWithImage:[UIImage imageNamed:@"cart_edit_bg"]];
    [bgView setFrame:CGRectMake(CONTAINER_WIDTH/2 -45, 25, 90, 35)];
    [self.containerView addSubview:bgView];
    
    self.numberLabel=[[UILabel alloc]initWithFrame:CGRectMake(CONTAINER_WIDTH/2 -33, 25, 66, 35)];
    ...
    [_subButton setImage:[UIImage imageNamed:@"cart_edit_sub_button"] forState:UIControlStateNormal];
    [_subButton setFrame:CGRectMake(CONTAINER_WIDTH/2 -85, 20, 45, 45)];
    [self.containerView addSubview:_subButton];
    
    self.addButton=[UIButton buttonWithType:UIButtonTypeCustom];
    [_addButton setImage:[UIImage imageNamed:@"cart_edit_add_button"] forState:UIControlStateNormal];
    [_addButton setFrame:CGRectMake(CONTAINER_WIDTH/2 +40, 20, 45, 45)];
    [self.containerView addSubview:_addButton];
    
    self.commitButton=[UIButton buttonWithType:UIButtonTypeCustom];
    [_commitButton setFrame:CGRectMake(CONTAINER_WIDTH/2 -38, 80, 77, 26)];
    [_commitButton setImage:[UIImage imageNamed:@"cart_edit_commit_button"] forState:UIControlStateNormal];
    [self.containerView addSubview:_commitButton];
}
...
-(void)setupCancelButton{
    ...
    CGFloat x = CGRectGetMaxX(self.containerView.frame) -10;
    CGFloat y = CGRectGetMinY(self.containerView.frame) -10;
    [_cancelButton setFrame:CGRectMake(x, y, 23, 23)];
    [_cancelButton setImage:[UIImage imageNamed:@"btn_close_gray"] forState:UIControlStateNormal];
    [_cancelButton addTarget:self action:@selector(dismm) forControlEvents:UIControlEventTouchUpInside];
...
//加入购物车时用的样式
-(void)setupAddCartAlertVIew{
    
    
}

// 内容分享布局
- (void)shareCenterLayout
{
    ...
    CGFloat height = SHARE_CONTAINER_HEIGHT;
    CGFloat left = (self.bounds.size.width - SHARE_CONTAINER_WIDTH) * 0.5;
    CGFloat top = (self.bounds.size.height - CONTENT_PADDING_BOTTOM - SHARE_CONTAINER_HEIGHT) ;
    self.containerView.transform = CGAffineTransformIdentity;
    self.containerView.frame = CGRectMake(left, top, SHARE_CONTAINER_WIDTH, height);
    ...
    if (self.items.count > 0) {
        ...
        y +=10;
            ...
            for (NSUInteger i = 0; i < self.buttons.count; i++) {
                ...
                button.frame = CGRectMake(20*(i+1)+i*50, y, 50, 66);
                if (self.buttons.count > 1) {
                    if (i == self.buttons.count - 1 && ((SIAlertItem *)self.items[i]).type == SHAlertVIewButtonTypeCancel) {
                        CGRect rect = CGRectMake(20, CGRectGetMaxY(button.frame)+20, 260, 44);
...
- (void)validateLayout
{
    ...
    if (_isTowButton) {
        [self setupAddCartAlertVIew];
        if (_hasCancelButton) {
            [self setupCancelButton];
        }
    }else{
        [self setupEditCartAlertView];
    }
    ...
    UIImageView *lineView = [[UIImageView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.messageLabel.frame), self.containerView.frame.size.width, 1)];
    lineView.image = [UIImage imageNamed:@"alert_line"];
    ...
    if (_isPassword) {
        ...
        rect.origin.y = 0;
        lineView.frame = rect;
        lineView.image = [UIImage imageNamed:@"alert_password"];
        [self.containerView insertSubview:lineView atIndex:0];
        
        self.passwordTextField.frame = CGRectMake(20, 40, 210, 35);
        self.passwordTextField.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
        [self.containerView addSubview:_passwordTextField];
    }
    ...
    if (self.items.count > 0) {
        ...
        if (self.items.count == 2) {
            ...
            CGFloat x = (self.containerView.bounds.size.width - BUTTON_WIDTH * 2 - GAP) * 0.5;
            UIButton *button = self.buttons[0];
            button.frame = CGRectMake(x, y, BUTTON_WIDTH, BUTTON_HEIGHT);
            button = self.buttons[1];
            button.frame = CGRectMake(x + BUTTON_WIDTH + GAP, y, BUTTON_WIDTH, BUTTON_HEIGHT);
        } else {
            for (NSUInteger i = 0; i < self.buttons.count; i++) {
                UIButton *button = self.buttons[i];
...
- (CGFloat)preferredHeight
{
    ...
    height += CONTENT_PADDING_BOTTOM*2;
	return height ;
}
...
- (UIButton *)buttonForItemIndex:(NSUInteger)index
{
	...
	switch (item.type) {
        ...
        case SHAlertVIewGrayButtonType:
            
            normalImage = [UIImage imageNamed:@"alert_button_gray"];
			[button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [button setTitleColor:[UIColor colorWithWhite:0.3 alpha:0.8] forState:UIControlStateHighlighted];
            
            break;
            
        case SHAlertVIewButtonType:
            
            normalImage = [UIImage imageNamed:@"fxls_button"];
			[button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [button setTitleColor:[UIColor colorWithWhite:0.3 alpha:0.8] forState:UIControlStateHighlighted];

            break;
            
        case SHAlertVIewPasswordType:
		...
		case SIAlertViewButtonTypeDestructive:
			...
			break;
        case SHAlertVIewButtonTypeSinaWeiBo:
			normalImage = [UIImage imageNamed:@"sns_sina_weibo100"];
			break;
        case SHAlertVIewButtonTypeTencentWeiBo:
			normalImage = [UIImage imageNamed:@"sns_qq_weibo100"];
            break;
        case SHAlertVIewButtonTypeWeiXin:
			normalImage = [UIImage imageNamed:@"sns_weixin_friend100"];
            break;
        case SHAlertVIewButtonTypeWeiXinGroup:
			normalImage = [UIImage imageNamed:@"sns_weixin100"];
            break;
        case SHAlertVIewButtonTypeCancel:
			normalImage = [UIImage imageNamed:@"sns_cancel_button"];
            break;
