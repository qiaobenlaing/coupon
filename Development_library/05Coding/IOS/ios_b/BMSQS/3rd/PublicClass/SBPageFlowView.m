...
- (void)layoutSubviews{
    ...
    if (_needsReload) {
        ...
        {
            //[_subViewCells release];
            UIView* v = [[_dataSource flowView:self cellForPageAtIndex:0]retain];
            [_subViewCells addObject:v];
            //[_subViewCells retain];
            [v release];
        }
...
- (void)initialize{
    ...
    _subViewCells = [[NSMutableArray alloc]init];
        ...
        case FlowViewOrientationHorizontal:{
            ...
            for (int i = _visibleRange.location; i < _visibleRange.location + _visibleRange.length; i++) {
                ...
                if (delta < _pageSize.width)
                {
                    ...
                    CGFloat insetHeight = (_pageSize.height * (1 - _minimumPageScale)) * (delta / _pageSize.height)/2.0;
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(insetHeight, inset, insetHeight, inset));
                    
                    UIView* cellView = (UIView*)[_subViewCells objectAtIndex:0];
                    for (UIView* v in cellView.subviews)
                    {
                        CGRect rect = v.frame;
                        //横向缩放比例
                        float scaleWidth = cell.frame.size.width / originCellFrame.size.width;
                        //纵向缩放比例
                        float scaleHeight = cell.frame.size.height / originCellFrame.size.height;
                        
                        for (UIView* tmpV in cell.subviews)
                        {
                            if (tmpV.tag == v.tag)
                            {
                                tmpV.frame = CGRectMake(rect.origin.x * scaleWidth, rect.origin.y * scaleHeight,rect.size.width * (scaleWidth), rect.size.height * (scaleHeight));
                            }
                        }
                ...
                else
                {
                    ...
                    CGFloat insetWidth = _pageSize.width * (1 - _minimumPageScale) / 2.0 ;
                    CGFloat insetHeight = _pageSize.height * (1 - _minimumPageScale) / 2.0;
                   
                    cell.frame = UIEdgeInsetsInsetRect(originCellFrame, UIEdgeInsetsMake(insetHeight, insetWidth, insetHeight, insetWidth));
                ...
                NSLog(@"rect%d,%f,%f,%f,%f",i , cell.frame.origin.x,cell.frame.origin.y, cell.frame.size.width, cell.frame.size.height);
            }
            break;
        }
        case FlowViewOrientationVertical:{
