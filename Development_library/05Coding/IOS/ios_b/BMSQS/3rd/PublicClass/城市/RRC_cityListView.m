...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        if (indexPath.section == 0)
        {
            ...
            if (appDelegate.currentCityName == nil || appDelegate.currentCityName.length <= 0 || [appDelegate.currentCityName rangeOfString:@"null"].length > 0)
            {
                [btn_head setTitle:[NSString stringWithFormat:@"不能定位到你所在的当前城市，请手动选择城市"] forState:UIControlStateNormal];
            }
            else
            {
                [btn_head setTitle:[NSString stringWithFormat:@"你所在的当前城市: %@",appDelegate.currentCityName] forState:UIControlStateNormal];
...
- (void)btnHeadClick
{
    ...
    SDZX_cityObject* obj = [citySingle getCityByName:appDelegate.currentCityName];
    appDelegate.currentCityCode = [[NSString alloc]initWithFormat:@"%@",obj.cityId];
    if (delegate != nil && [delegate respondsToSelector:@selector(selectCity:)])
    {
        [delegate selectCity:obj];
