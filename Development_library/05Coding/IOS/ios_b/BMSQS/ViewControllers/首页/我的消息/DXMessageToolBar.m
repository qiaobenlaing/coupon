...
//    allButtonWidth += CGRectGetWidth(self.moreButton.frame) + kHorizontalPadding * 2.5;
    ...
    // 初始化输入框
    self.inputTextView = [[XHMessageTextView  alloc] initWithFrame:CGRectMake(10, kVerticalPadding, APP_VIEW_WIDTH-100, kInputTextViewMinHeight)];
    ...
    //表情
    self.faceButton = [[UIButton alloc] initWithFrame:CGRectMake(self.inputTextView.frame.origin.x+self.inputTextView.frame.size.width+10, kVerticalPadding,70, kInputTextViewMinHeight)];
    [self.faceButton setTitle:@"发送" forState:UIControlStateNormal];
    self.faceButton.backgroundColor = UICOLOR(182, 0, 12, 1.0);
    ...
    [self.faceButton addTarget:self action:@selector(sendMess) forControlEvents:UIControlEventTouchUpInside];
//    [self.faceButton setImage:[UIImage imageNamed:@"IWStatusSendIconEmoji.png"] forState:UIControlStateNormal];
//    [self.faceButton setImage:[UIImage imageNamed:@"chatBar_faceSelected"] forState:UIControlStateHighlighted];
//    [self.faceButton setImage:[UIImage imageNamed:@"IWStatusSendIconKeyboard.png"] forState:UIControlStateSelected];
