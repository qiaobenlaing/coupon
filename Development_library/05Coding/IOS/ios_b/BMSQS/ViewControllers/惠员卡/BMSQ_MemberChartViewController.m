...
//    self.navTitle = @"给惠圈的建议";
    if (self.titleNav!=nil) {
        [self setNavTitle:self.titleNav];
    ...
    self.tableView=[[UITableView alloc]initWithFrame:CGRectMake(0, APP_VIEW_ORIGIN_Y, self.view.frame.size.width, self.view.frame.size.height-60) style:UITableViewStylePlain];
    ...
    if ([self.userID  isEqual: APP_HQ_CODE]) {
        [self.navigationItem setTitle:@"对惠圈的建议"];
        [self setNavTitle:@"给惠圈留言"];
        self.tableView.frame = CGRectMake(0, APP_VIEW_ORIGIN_Y, self.view.frame.size.width, APP_VIEW_CAN_USE_HEIGHT);
        self.keyBordView.frame = CGRectMake(0, APP_VIEW_HEIGHT-60, self.view.frame.size.width, 60);
    }
...
-(void)KeyBordView:(KeyBordVIew *)keyBoardView textFiledReturn:(UITextField *)textFiled
{
    ...
    [uD objectForKey:@""];
    chartMessage.icon=[self.shopInfo objectForKey:@"logoUrl"];
...
- (void)httpRequest:(NSString *)str
{
    ...
    NSDictionary *dic = @{@"tokenCode":[gloabFunction getUserToken],
                          @"vcode":vcode,
                          @"shopCode":[gloabFunction getShopCode],
...
- (void)httpContentRequest:(BOOL)isAll
{
    ...
    NSString* vcode = [gloabFunction getSign:@"getMsg" strParams:self.userID];
...
{
    NSString* vcode = [gloabFunction getSign:@"countUnreadMsg" strParams:self.userID];
...
{
    NSString* vcode = [gloabFunction getSign:@"readMsg" strParams:self.userID];
...
- (void)sGetShopInfo {
    ...
    [self.jsonPrcClient invokeMethod:@"sGetShopInfo" withParameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        weakSelf.shopInfo = responseObject[@"shopInfo"];
