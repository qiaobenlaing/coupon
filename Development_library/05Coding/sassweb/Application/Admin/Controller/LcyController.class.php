...
//        $this->pager = new Pager(I('page', 1, 'intval'), I('pageSize', Pager::DEFUALT_PAGE_SIZE, 'intval'));
	...
	public function Daipay(){
		...
		require_once "/usr/local/nginx/html/hfq.huift.com.cn/huiquan/example/jsapi.php";
    ...
    public function weixinRefund() {
        ...
        $weixinInfo=require_once "/usr/local/nginx/html/hfq.huift.com.cn/huiquan/example/refund.php";
        if($weixinInfo['return_code']=='SUCCESS'&&$weixinInfo['result_code']=='SUCCESS'){
            ...
            foreach($tuiCouponNbr as $val){
                $data->status=0;
                $userCouponModel->where("userCouponNbr='$val'")->save($data);//用户优惠券
                $data2->status=11;
                $OrderCoupon->where("couponCode='$val'")->save($data2);
                $userConsumeModel->where(array("orderCode"=>$orderCode))->save(array("status"=>7));
            }
            $consumeOrderMdl = new ConsumeOrderModel();
            $consumeOrderMdl->where(array("orderCode"=>$orderCode))->save(array("status"=>7));//订单表
	...
	public function weiPay(){
		...
		require_once "/usr/local/nginx/html/hfq.huift.com.cn/huiquan/example/weiPay.php";
