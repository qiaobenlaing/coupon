...
class ShopController extends ApiBaseController {
    ...
    public function getBillStatistics($shopCode, $timeLimit, $billType, $searchWord) {
        ...
        if($billType == 1) { // 顾客清单
            ...
            $data = $this->getUserConsumeStatistics($shopCode, $time, $searchWord);
        } else {
            $data = array();
        }
        return $data;
...
//        }
    ...
    public function getConsumeInfo($consumeCode) {
        $userConsumeMdl = new UserConsumeModel();
        // 获得账单信息
        $consumeInfo = $userConsumeMdl->getConsumeInfo(array('consumeCode' => $consumeCode), array('realPay', 'deduction', 'couponUsed', 'shopBonus', 'bankCardDeduction', 'platBonus', 'usedUserCouponCode', 'isCard', 'consumerCode', 'location', 'Shop.shopName','ConsumeOrder.orderNbr','ConsumeOrder.orderAmount','ConsumeOrder.orderTime', 'Shop.shopCode', 'firstDeduction', 'couponDeduction', 'cardDeduction', 'UserConsume.status', 'User.mobileNbr', 'avatarUrl', 'nickName', 'userCode', 'payType', 'noDiscountPrice', 'consumeTime'));
