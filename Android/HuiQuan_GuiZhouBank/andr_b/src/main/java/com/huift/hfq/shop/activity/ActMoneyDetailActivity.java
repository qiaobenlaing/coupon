...
import com.huift.hfq.shop.fragment.ActListMoneyFragment;
...
public class ActMoneyDetailActivity extends Activity {
	...
	private void init() {
		...
		//领取人数
		mProgressBar = (ProgressBar)findViewById(R.id.progress_horizontal_receivenumber);
		//进度条的说明文字
		mTvDrawBouns = (TextView)findViewById(R.id.tv_actmoneydetail_receiver);
		//传过来的code
		Intent intent = this.getIntent();
		mMoneyCode = intent.getStringExtra(ACTMONEY_CODE);
		Log.d(TAG, "mMoneyCode========="+mMoneyCode);
		mBounsSorce = getIntent().getStringExtra(BOUNS_SOURCE);
		Log.d(TAG, "mBounsSorce="+mBounsSorce);
		//红包详情
		getBonusInfo();
	}
	...
	private void getBonusInfo(){
		...
		new getBonusInfoTask(this, new getBonusInfoTask.Callback() {
			@Override
			public void getResult(JSONObject result) {
				mTvEdit.setEnabled(true);
				//领取人数
				ProgressBar pgDrawBouns = (ProgressBar)findViewById(R.id.pg_drawbouns);
				//进度条的说明文字
				TextView tvDrawBouns = (TextView)findViewById(R.id.tv_drawbouns);
	...
	public void trunIdenCode(final View view){
		switch (view.getId()) {
		case R.id.tv_msg: //停发
			final String stop = getString(R.string.tv_actmoney_user); // 停发
			final String enable = getString(R.string.tv_actmoney_enable); //启用
			final String edtStop = mTvEdit.getText().toString();
			if (edtStop.equals(stop)) { // 文本等于停发
				PopupWindowUtils.showDialog(this, "温馨提示", "确定要停用吗?", "确定", "取消", new OnResultListener() {
					@Override
					public void onOK() {
						mTvEdit.setEnabled(false);
		...
		case R.id.layout_turn_in: //返回
			Intent intent = new Intent(ActMoneyDetailActivity.this, ActListActivity.class);
	...
	private void finishClick () {
		Intent intent = null;
		Log.d(TAG, "mBounsSorceaa========"+mBounsSorce);
		if (mBounsSorce.equals(LIST_BONUS)) {
			intent = new Intent(this, ActListActivity.class);
		} 
		intent.putExtra(ActListMoneyFragment.MONEY_STATUS, mTvEdit.getText().toString());
		String bounsCode = "";
		if (mBouns != null) {
	...
	private void getBounsStatus (String status,final String stop,final String enable) {
		mTvEdit.setEnabled(false);
		new ChangeBonusStatusTask(this, new ChangeBonusStatusTask.Callback() {
