	...
	private void  getTable (final View v) {
		if ("20".equals(mOrderInfo.getOrderType())) {//门店订单
			 if (String.valueOf(Util.NUM_ONE).equals(mOrderInfo.getTableNbrSwitch())) {//开启桌号管理
				 DialogUtils.showInputDialog(getActivity(), getString(R.string.myafter_order__table_title),getString(R.string.input_table_no), getString(R.string.ok), getString(R.string.no), new DialogUtils().new OnResListener() {
						@Override
