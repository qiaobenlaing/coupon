...
public class StartActivity extends Activity {
    ...
    private void init() {
        ...
        boolean isFirstRun = DB.getBoolean(DB.Key.SHOP_IS_FIRST_RUN);
        if (isFirstRun) {//若为第一次使用
            DB.saveBoolean(DB.Key.SHOP_IS_FIRST_RUN, false);
            ...
            new Handler().postDelayed(new Runnable() {
                ...
                public void run() {
                    //得到登录的用户名和密码
                    SharedPreferences mSharedPreferences = getSharedPreferences(ShopConst.LOGIN_KEEP, Context.MODE_PRIVATE);
                    ...
                    if (Util.isEmpty(mobileNbr) || Util.isEmpty(password)) {
                        ...
                        if (userToken == null) {
                            ...
                            mZoneId = userToken.getZoneId();
                            if (mobileNbr.equals(mUserName) && password.equals(mUserPwd) && zoneId.equals(mZoneId)) {
                                //是否联网状态
                                if (Util.isNetworkOpen(StartActivity.this)) {
                                    ...
                                    }, HomeActivity.class, Const.Login.ANDR_B_HOMEFRAGMENT).execute(mobileNbr, password, zoneId, regId);
                                } else {
