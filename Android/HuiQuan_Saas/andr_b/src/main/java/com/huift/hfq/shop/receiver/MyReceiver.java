...
public class MyReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
            ...
            if (ShopConst.ShopMessage.INVITE_SHOP.equals(mMessage)) {//邀请入驻惠圈
        		...
        		String isHome = DB.getStr(ShopConst.Key.IS_HOME_MAIN); // 判断是否在首页运行
        		if (ShopConst.Key.IS_HOME_MAIN.equals(isHome)) {
        			showNotification (context,idx,i); // 是在首页运行
        		} else {
	...
	//支付
	private void showNotification (Context context,int idx,Intent iten) {
		...
		Notification notification=new Notification.Builder(context)
				...
				.setContentTitle("惠圈商户")
				.setContentText("你有新的支付信息"+ (++msgNum)+"条")
	...
	//添加商品
	private void addproduct (Context context,int idx,Intent i) {
		PendingIntent contentIntent = PendingIntent.getActivity(context, idx, i, PendingIntent.FLAG_UPDATE_CURRENT);
		Notification notification=new Notification.Builder(context)
				.setSmallIcon(R.drawable.ic_launcher)
				.setContentTitle("惠圈商户")
				.setContentText("你有新的邀请添加商品展示信息"+ (++msgNum)+"条")
				.setContentIntent(contentIntent)
				.getNotification();
		notification.flags |= Notification.FLAG_AUTO_CANCEL; // 点击后自动清除图标
		notification.defaults = Notification.DEFAULT_SOUND; // 通知默认的声音
	...
	//报名
	private void showAct (Context context,int idx,Intent i) {
		PendingIntent contentIntent = PendingIntent.getActivity(context, idx, i, PendingIntent.FLAG_UPDATE_CURRENT);
		Notification notification=new Notification.Builder(context)
				.setSmallIcon(R.drawable.ic_launcher)
				.setContentTitle("惠圈商户")
				.setContentText("你有新的报名信息" + (++joinNum)+ "条")
				.setContentIntent(contentIntent)
				.getNotification();
		notification.flags |= Notification.FLAG_AUTO_CANCEL; // 点击后自动清除图标
		notification.defaults = Notification.DEFAULT_SOUND; // 通知默认的声音
	...
	//退出活动
	private void showExitAct (Context context,int idx,Intent i) {
		PendingIntent contentIntent = PendingIntent.getActivity(context, idx, i, PendingIntent.FLAG_UPDATE_CURRENT);
		Notification notification=new Notification.Builder(context)
				.setSmallIcon(R.drawable.ic_launcher)
				.setContentTitle("惠圈商户")
				.setContentText("你有新的活动退出信息" + (++ExitNum)+ "条")
				.setContentIntent(contentIntent)
				.getNotification();
		notification.flags |= Notification.FLAG_AUTO_CANCEL; // 点击后自动清除图标
		notification.defaults = Notification.DEFAULT_SOUND; // 通知默认的声音
	...
	private void showLogin(){
		DialogUtils.show(mLoginTime,new Runnable() {
			...
			public void run() {
				...
				}).execute(mUserName,mUserPwd,zoneId,registerId);
			}
		});
	}
