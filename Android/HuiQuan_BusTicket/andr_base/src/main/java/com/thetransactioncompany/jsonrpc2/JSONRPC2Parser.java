...
public class JSONRPC2Parser {
	...
	/** data name in resp json */
	public static final String RESP_RESULT_NAME = "result";
	/** error name in resp json */
	public static final String RESP_ERROR_NAME = "error";
	...
	public JSONRPC2Response parseJSONRPC2Response(final String jsonString)
		...
		if (jsonObject.containsKey(JSONRPC2Parser.RESP_RESULT_NAME) && ! jsonObject.containsKey(JSONRPC2Parser.RESP_ERROR_NAME)) {
			
			// Success
			Object res = jsonObject.remove(JSONRPC2Parser.RESP_RESULT_NAME);
			
			response = new JSONRPC2Response(res, id);
					
		}
		else if (! jsonObject.containsKey(JSONRPC2Parser.RESP_RESULT_NAME) && jsonObject.containsKey(JSONRPC2Parser.RESP_ERROR_NAME)) {
		
			// Error JSON object
			Object errorJSON = jsonObject.remove(JSONRPC2Parser.RESP_ERROR_NAME);
